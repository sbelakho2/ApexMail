...
impl<'a> MarketingHeader<'a> {
    pub fn render_html(&self) -> String {
        let dropdown_markup = if self.active_dropdown == Some("Product") {
            "<div class=\"animate-in absolute top-full left-0 pt-2\"><div class=\"w-80 p-2 bg-white rounded-xl border border-surface-200 shadow-xl shadow-surface-900/5\"><a href=\"/features\" class=\"flex items-start gap-3 p-3 rounded-lg hover:bg-surface-50 transition-colors group\"><div><div class=\"font-semibold text-surface-900 text-sm mb-0.5\">Features</div><div class=\"text-xs text-surface-500 leading-snug\">Explore our complete feature set</div></div></a><a href=\"/compliance\" class=\"flex items-start gap-3 p-3 rounded-lg hover:bg-surface-50 transition-colors group\"><div><div class=\"font-semibold text-surface-900 text-sm mb-0.5\">Security</div><div class=\"text-xs text-surface-500 leading-snug\">Enterprise-grade protection</div></div></a><a href=\"/private-cloud\" class=\"flex items-start gap-3 p-3 rounded-lg hover:bg-surface-50 transition-colors group\"><div><div class=\"font-semibold text-surface-900 text-sm mb-0.5\">Private Cloud</div><div class=\"text-xs text-surface-500 leading-snug\">Single-tenant deployments</div></div></a></div></div>"
        } else {
            ""
        };
        let mobile_menu = if self.mobile_menu_open {
            "<div class=\"animate-in lg:hidden bg-white border-b border-surface-200 overflow-hidden\"><div class=\"px-4 py-6 space-y-6\"><div class=\"space-y-3\"><div class=\"text-xs font-bold text-surface-900 px-3\">Product</div><div class=\"space-y-1\"><a href=\"/features\" class=\"flex items-center gap-3 p-3.5 rounded-lg hover:bg-surface-50 transition-colors\">Features</a><a href=\"/compliance\" class=\"flex items-center gap-3 p-3.5 rounded-lg hover:bg-surface-50 transition-colors\">Security</a><a href=\"/private-cloud\" class=\"flex items-center gap-3 p-3.5 rounded-lg hover:bg-surface-50 transition-colors\">Private Cloud</a></div></div><a href=\"/pricing\" class=\"block px-3 py-3 text-surface-900 font-medium hover:bg-surface-50 rounded-lg text-sm\">Pricing</a><a href=\"/compliance\" class=\"block px-3 py-3 text-surface-900 font-medium hover:bg-surface-50 rounded-lg text-sm\">Compliance</a><a href=\"/private-cloud\" class=\"block px-3 py-3 text-surface-900 font-medium hover:bg-surface-50 rounded-lg text-sm\">Enterprise</a><div class=\"pt-6 border-t border-surface-200 space-y-3 px-3\"><a href=\"https://app.apexmail.ee/login\" class=\"block w-full text-center py-2.5 text-surface-900 font-semibold border border-surface-200 rounded-lg hover:bg-surface-50 transition-colors text-sm\">Sign In</a><a href=\"https://app.apexmail.ee/signup\" class=\"block w-full btn-primary text-center py-2.5 text-sm\">Get Started Free</a></div></div></div>"
        } else {
            ""
        };

        format!(
            "<header class=\"fixed top-0 left-0 right-0 z-50 transition-all duration-300 {}\"><nav class=\"max-w-7xl mx-auto px-4 sm:px-6 lg:px-8\"><div class=\"flex items-center justify-between h-16 lg:h-20\"><a href=\"/\" class=\"flex items-center gap-2 group\"><div class=\"w-8 h-8 rounded-lg bg-primary-600 flex items-center justify-center transition-transform group-hover:scale-105 shadow-sm\"><span class=\"w-5 h-5 text-white\">⚡</span></div><span class=\"text-xl font-bold text-surface-900 tracking-tight\">ApexMail</span></a><div class=\"hidden lg:flex items-center gap-2\"><div class=\"relative\"><button class=\"flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors rounded-lg hover:bg-surface-50\" aria-haspopup=\"menu\" aria-expanded=\"{}\">Product<span class=\"w-4 h-4\">⌄</span></button>{}</div><a href=\"/docs\" class=\"px-3 py-2.5 text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors rounded-lg hover:bg-surface-50\">Documentation</a><a href=\"/api-console\" class=\"px-3 py-2.5 text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors rounded-lg hover:bg-surface-50\">API Console</a><a href=\"/pricing\" class=\"px-3 py-2.5 text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors rounded-lg hover:bg-surface-50\">Pricing</a><a href=\"/compliance\" class=\"px-3 py-2.5 text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors rounded-lg hover:bg-surface-50\">Compliance</a><a href=\"/private-cloud\" class=\"px-3 py-2.5 text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors rounded-lg hover:bg-surface-50\">Enterprise</a></div><div class=\"hidden lg:flex items-center gap-3\"><a href=\"https://app.apexmail.ee/login\" class=\"text-sm font-semibold text-surface-600 hover:text-surface-900 transition-colors px-3 py-2.5 hover:bg-surface-50 rounded-lg\">Sign In</a><a href=\"https://app.apexmail.ee/signup\" class=\"btn-primary text-sm\">Get Started Free</a></div><button class=\"lg:hidden p-2 text-surface-600 hover:text-surface-900 rounded-lg hover:bg-surface-50 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center\" aria-label=\"{} menu\" aria-expanded=\"{}\">{}</button></div></nav>{}</header>",
            if self.is_scrolled {
...
impl<'a> MarketingApiConsole<'a> {
    pub fn render_html(&self) -> String {
        ...
        let sidebar = API_CONSOLE_ENDPOINTS
            ...
            .map(|(index, candidate)| {
                format!(
                    "<button class=\"w-full text-left px-3 py-2.5 text-sm flex items-center gap-2 transition-colors {}\"><span class=\"font-mono text-xs font-bold {}\">{}</span><span class=\"truncate\">{}</span></button>",
        ...
        let request_editor = if endpoint.method == "POST" {
            format!(
                "<div class=\"px-4 py-3 border-b border-gray-700 bg-gray-800/20\"><div class=\"text-xs font-semibold text-gray-400 mb-2\">Request Body</div><textarea class=\"w-full bg-gray-900 text-gray-200 font-mono text-xs rounded-lg p-3 border border-gray-700 focus:border-primary-500 focus:outline-none resize-none\" rows=\"4\" spellcheck=\"false\">{}</textarea></div>",
                self.request_body,
            )
        } else {
            String::new()
        ...
        let response_markup = if self.loading {
            "<div class=\"flex items-center gap-2 text-gray-400 text-sm\"><div class=\"w-4 h-4 border-2 border-primary-500 border-t-transparent rounded-full animate-spin\"></div> Sending request...</div>".to_string()
        } else if let Some(response) = self.response {
            format!(
                "<pre class=\"text-xs text-emerald-400 font-mono bg-gray-800/50 rounded-lg p-3 overflow-auto max-h-60\">{}</pre>",
                response,
            )
        } else {
