...
impl Http2Fingerprint {
    ...
    pub fn compute(preface: &Http2Preface) -> Self {
        ...
        let settings_str: String = preface
            .settings_order
            .iter()
        ...
        let frame_str: String = preface.frame_order.iter().map(|f| f.to_char()).collect();
        let frame_order_fingerprint = truncated_sha256(&frame_str, 8);

        // Priority fingerprint
        let priority_str: String = preface
            .priorities
            .iter()
            .map(|p| {
                format!(
        ...
        let fingerprint = format!(
            "{}|{}|{}",
            settings_fingerprint, frame_order_fingerprint, priority_fingerprint
        );
    ...
    pub fn likely_browser(&self) -> Option<&'static str> {
        ...
        match self.settings_fingerprint.as_str() {
            // Chrome typically:HEADER_TABLE_SIZE=65536, ENABLE_PUSH=0, // INITIAL_WINDOW_SIZE=6291456, MAX_HEADER_LIST_SIZE=262144
