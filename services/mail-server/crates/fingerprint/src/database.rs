...
use crate::{Ja4Fingerprint, Http2Fingerprint};
...
// Chrome on macOS
        self.known.insert(
            ...
            FingerprintClassification::benign(
                ClientIdentity::Browser { name: "Chrome".to_string(), version: "120+".to_string() },
...
// Firefox
        self.known.insert(
            "t120511h1".to_string(),
            FingerprintClassification::benign(
                ClientIdentity::Browser { name: "Firefox".to_string(), version: "120+".to_string() },
...
// Safari
        self.known.insert(
            "t130815h2".to_string(),
            FingerprintClassification::benign(
                ClientIdentity::Browser { name: "Safari".to_string(), version: "17+".to_string() },
...
/// Get observation for a fingerprint
    pub fn get_observation(&self, fingerprint: &CombinedFingerprint) -> Option<ObservedFingerprint> {
        let key = self.fingerprint_key(fingerprint);
...
/// Add a known fingerprint classification
    pub fn add_classification(&self, fingerprint_prefix: &str, classification: FingerprintClassification) {
        self.known.insert(fingerprint_prefix.to_string(), classification);
    }
...
// Find oldest entries and remove them
            let mut entries: Vec<_> = self.observed.iter()
