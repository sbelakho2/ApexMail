...
mod ja4_tests {
    ...
    fn test_common_cipher_suites_not_grease() {
        ...
        for &cipher in &ciphers {
            assert!(
                !is_grease_value(cipher),
                "Cipher 0x{:04x} should not be GREASE",
                cipher
            );
        }
    }
...
mod http2_tests {
    ...
    fn test_setting_enum_conversion() {
        assert_eq!(
            Http2Setting::from_u16(0x1),
            Some(Http2Setting::HeaderTableSize)
        );
        assert_eq!(
            Http2Setting::from_u16(0x4),
            Some(Http2Setting::InitialWindowSize)
        );
