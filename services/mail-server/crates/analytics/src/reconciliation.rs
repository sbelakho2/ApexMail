...
impl ReconciliationWorker {
    ...
    async fn check_health(&self) -> anyhow::Result<serde_json::Value> {
        ...
        // Queue backlog
        let queue_backlog: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM messages WHERE status = 'queued'")
                .fetch_one(&self.pool)
                .await
                .map_err(|e| anyhow::anyhow!("reconciliation queue_backlog query failed: {e}"))?;

        let healthy =
            orphaned_count.0 < 100 && event_lag.0.unwrap_or(0.0) < 300.0 && queue_backlog.0 < 1000;
