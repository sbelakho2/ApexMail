...
fn test_cc_luhn_valid_visa_detected() {
    ...
    assert!(cc, "Luhn-valid Visa number must be detected. Findings: {:?}", verdict.pii_findings);
...
fn test_cc_detected_external_recipient() {
    let e = engine();
    let verdict = e.scan("Attach card 4532015112830366 to your account.", Some("external-corp.com"));
    let cc = verdict.pii_findings.iter().any(|f| matches!(f.pii_type, PiiType::CreditCard));
...
fn test_ssn_dashed_format_detected() {
    ...
    let ssn = verdict.pii_findings.iter().any(|f| matches!(f.pii_type, PiiType::Ssn));
    assert!(ssn, "Dashed SSN must be detected. Findings: {:?}", verdict.pii_findings);
}
...
fn test_low_entropy_text_no_entropy_finding() {
    let e = engine();
    let verdict = e.scan_body("Dear team, please review the quarterly report and provide your feedback by Friday.");
    assert!(
        verdict.entropy_findings.is_empty(),
...
fn test_clean_business_email_allowed() {
    ...
    assert!(verdict.pii_findings.is_empty(), "Clean email must have zero PII findings");
}
...
fn test_product_code_not_ssn() {
    ...
    let ssn = verdict.pii_findings.iter().any(|f| matches!(f.pii_type, PiiType::Ssn));
    assert!(!ssn, "Malformed SSN-like pattern with wrong digit count must NOT be detected");
}
