...
// high-entropy, so catch them separately from the token splitter above.
    ...
    for line in text.lines() {
        if let Some((raw_key, raw_value)) = line.split_once('=') {
            ...
            if is_secret_assignment_key(&key) && value.len() >= 12 && looks_like_assigned_secret_value(value) {
                let entropy = shannon_entropy(value);
                let risk = if key.contains("database_url") || key.contains("password") { 3.0 } else { 2.0 };
                ...
                if !findings.iter().any(|finding| finding.token_preview == preview) {
                    findings.push(EntropyFinding {
                        token_preview: preview,
                        entropy,
                        length: value.len(),
...
fn looks_like_assigned_secret_value(value: &str) -> bool {
    ...
    let has_symbol = value.chars().any(|c| matches!(c, '_' | '-' | '/' | ':' | '@' | '.'));
...
// Equal distribution of many characters = high entropy
    ...
    fn test_detect_env_style_secret_assignments() {
        ...
        assert!(!result.findings.is_empty(), "Should detect env-style secret assignments");
    }
