...
mod hashmap_perf_tests {
    ...
    fn test_ip_lookup_performance() {
        ...
        assert!(
            ...
            "Should handle >100k lookups/sec"
        );
    }
    ...
    fn test_concurrent_hashmap_simulation() {
        ...
        let mut maps: Vec<HashMap<String, u64>> = (0..8).map(|_| HashMap::new()).collect();
...
mod fingerprint_perf_tests {
    ...
    fn test_ja4_format_generation() {
        ...
        assert!(
            fps_per_sec > 50_000.0,
            "Should generate >50k fingerprints/sec"
        );
    }
}
...
mod decision_latency_tests {
    ...
    fn test_decision_latency() {
        ...
        assert!(
            decisions_per_sec > 100_000.0,
            "Should handle >100k decisions/sec"
        );
    }
...
mod memory_tests {
    ...
    fn test_ip_entry_size() {
        ...
        println!(
            "100k IPs would use: {} MB",
            entry_size * 100_000 / 1_000_000
        );
...
mod concurrent_perf_tests {
    ...
    fn test_simulated_concurrent_load() {
        ...
        let total_count: u64 = counters.iter().map(|c| c.load(Ordering::Relaxed)).sum();
...
mod anomaly_perf_tests {
    ...
    fn test_anomaly_score_throughput() {
        ...
        assert!(
            scores_per_sec > 1_000_000.0,
            "Should calculate >1M scores/sec"
        );
    }
    ...
    fn test_feature_extraction_performance() {
        ...
        assert!(
            extractions_per_sec > 1_000_000.0,
            "Should extract >1M features/sec"
        );
    }
}
