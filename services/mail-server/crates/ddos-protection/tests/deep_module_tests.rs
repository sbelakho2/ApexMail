...
mod bug_fix_verification {
    ...
    fn session_first_iat_not_polluted() {
        ...
        assert_eq!(
            cov, 1.0,
            "CoV should be default with insufficient data: {}",
            cov
        );
    }
    ...
    fn session_endpoints_bounded_memory() {
        ...
        assert!(
            info.endpoint_diversity > 0.9,
            "With all unique endpoints, diversity should be high: {}",
            info.endpoint_diversity
        );
    }
    ...
    fn cost_limiter_refill_logic_correct() {
        ...
        let decision = limiter.check(
            "tenant1",
            "/v1/health",
            Some(RequestCost::new(900, 0, 0, 0)),
        );
...
mod session_tests {
    ...
    fn session_tracks_requests_per_minute() {
        ...
        assert!(
            info.requests_per_minute > 100.0,
            "RPM should be high: {}",
    ...
    fn session_tracks_errors() {
        ...
        assert!(
            info.error_rate > 0.6 && info.error_rate < 0.7,
            "Error rate should be ~0.666: {}",
            info.error_rate
        );
    }
...
mod decision_tests {
    ...
    fn pow_challenge_verification_correct_nonce() {
        ...
        assert!(
            found_nonce.is_some(),
            "Should find a valid nonce for difficulty 4"
        );
    }
...
mod cost_tests {
    ...
    fn cost_tenant_quota_exhaustion() {
        ...
        let d2 = limiter.check(
            "tenant1",
            "/v1/messages/:id",
            Some(RequestCost::new(1000, 0, 0, 0)),
        );
    ...
    fn cost_system_overload() {
        ...
        let decision = limiter.check(
            "tenant1",
            "/v1/messages/:id",
            Some(RequestCost::new(200, 0, 0, 0)),
        );
    ...
    fn cost_different_tenants_isolated() {
        ...
        let _ = limiter.check(
            "tenant1",
            "/v1/messages/:id",
            Some(RequestCost::new(2000, 0, 0, 0)),
        );
