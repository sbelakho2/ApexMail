...
mod smtp_attacks {
    ...
    /// BUG FIX VERIFICATION: Tarpit delay must escalate progressively.
    ...
    fn tarpit_escalates_progressively_under_sustained_invalid_commands() {
        ...
        for i in 0..3 {
            ...
            assert_eq!(action, SmtpAction::Reject("500 Unrecognized command"),
                "Command {} should be rejected, not tarpitted", i);
        }
        assert_eq!(prot.invalid_commands(), 3);
        assert!(prot.should_tarpit().is_none(), "At max_invalid, not over — no tarpit");

        // 4th invalid: 1 over limit → 1 * 2s = 2s
        let action = prot.process_command("JUNK4").unwrap();
        assert_eq!(action, SmtpAction::Tarpit(Duration::from_secs(2)),
        ...
        assert_eq!(action, SmtpAction::Tarpit(Duration::from_secs(4)),
        ...
        assert_eq!(prot.should_tarpit(), Some(Duration::from_secs(14)),
    ...
    /// BUG FIX VERIFICATION: check_data_rate uses float division.
    #[test]
    ...
    /// BUG FIX VERIFICATION: unregister_connection cannot underflow the atomic counter.
    #[test]
    ...
    fn connection_rate_limit_enforced_within_window() {
        ...
        assert!(matches!(result, Err(SmtpProtectionError::ConnectionRateLimited)),
...
mod adaptive_attacks {
    ...
    /// BUG FIX VERIFICATION: With zero std dev, an attack spike is still detected.
    ...
    fn attack_detected_against_constant_rate_baseline() {
        ...
        assert!(stats.rps_std < 0.001, "Baseline std should be ~0: {}", stats.rps_std);
    ...
    fn gradual_ramp_then_spike_detected() {
        ...
        assert!(limiter.current_threshold() < pre,
            "Threshold must tighten during attack: {} should be < {}", limiter.current_threshold(), pre);
    }
    ...
    fn ema_converges_toward_ideal_threshold() {
        ...
        assert!(threshold >= 250 && threshold <= 350,
            "Threshold should converge near 300 (200 * 1.5), got: {}", threshold);
    }
    ...
    fn threshold_clamped_under_extreme_traffic() {
        ...
        assert!(limiter.current_threshold() <= 500,
            "Threshold must not exceed max: {}", limiter.current_threshold());
        ...
        assert!(limiter.current_threshold() >= 50,
            "Threshold must not drop below min: {}", limiter.current_threshold());
    }
}
...
mod bot_attacks {
    ...
    /// BUG FIX VERIFICATION: First inter-arrival time is no longer polluted by
    ...
    fn first_request_does_not_pollute_timing_analysis() {
        ...
        assert!(assessment.signals.timing_regularity >= 0.7,
            "Bot with regular timing should score high: {}", assessment.signals.timing_regularity);
    }

    /// BUG FIX VERIFICATION: Endpoint concentration is windowed and doesn't 
    /// dilute over time. A bot that hits diverse endpoints early but then 
    /// concentrates should be detected.
    #[test]
    fn endpoint_concentration_is_windowed_not_cumulative() {
        ...
        assert!(assessment1.signals.endpoint_concentration < 0.5,
        ...
        assert!(assessment2.signals.endpoint_concentration >= 0.7,
    ...
    fn credential_stuffing_bot_detected() {
        ...
        assert!(assessment.signals.endpoint_concentration >= 0.7,
            "Single endpoint should have high concentration: {}",
            assessment.signals.endpoint_concentration);
        assert!(assessment.signals.error_anomaly >= 0.5,
            "98% error rate should flag: {}", assessment.signals.error_anomaly);
        assert!(assessment.bot_probability > 0.5,
    ...
    fn normal_human_browsing_scores_low() {
        ...
        let endpoints = [
            "/", "/api/messages", "/api/templates", "/api/domains",
            "/api/settings", "/api/analytics", "/api/users",
            "/api/billing", "/api/keys", "/api/logs",
        ];
        ...
        let assessment = behavior.analyze();
        assert!(assessment.has_sufficient_data);
        assert!(assessment.signals.endpoint_concentration < 0.5,
            "Diverse browsing should have low concentration: {}",
            assessment.signals.endpoint_concentration);
        assert!(assessment.signals.error_anomaly < 0.5,
            "Low error rate should score low: {}", assessment.signals.error_anomaly);
    }
    ...
    fn zero_error_rate_with_high_volume_is_suspicious() {
        ...
        assert!(assessment.signals.error_anomaly >= 0.4,
    ...
    fn sequence_entropy_distinguishes_bot_from_human() {
        ...
        assert!(bot_entropy < human_entropy,
            "Bot entropy ({}) should be lower than human entropy ({})",
            bot_entropy, human_entropy);
        assert!(bot_entropy < 0.5, "Bot entropy should be very low: {}", bot_entropy);
        assert!(human_entropy > 1.0, "Human entropy should be higher: {}", human_entropy);
    }
}
...
mod cost_attacks {
    ...
    /// BUG FIX VERIFICATION: system_budget does not underflow to u64::MAX
    ...
    fn system_budget_does_not_underflow_via_record_cost() {
        ...
        assert_eq!(remaining, 0,
            "System budget should be 0, not u64::MAX. Got: {}", remaining);
    }

    /// BUG FIX VERIFICATION: check() deduction doesn't underflow under concurrent load
    #[test]
    fn system_budget_doesnt_underflow_via_check() {
        ...
        assert!(remaining <= 100,
            "System budget should be at or near 0, got: {}", remaining);
        // Critically: it should never be u64::MAX or near it
        assert!(remaining < 1_000_000,
            "System budget must not underflow to huge value: {}", remaining);
    }
    ...
    fn tenant_isolation_under_exhaustion() {
        ...
        assert!(matches!(result, CostDecision::QuotaExceeded { .. }),
    ...
    fn expensive_endpoints_consume_budget_faster() {
        ...
        assert!(count <= 5,
            "Expensive batch endpoint should exhaust budget in ~3 requests, did: {}", count);
        assert!(count >= 2,
            "Should allow at least 2 batch requests, did: {}", count);
    }
    ...
    fn custom_cost_overrides_registry() {
        ...
        assert!(matches!(result, CostDecision::Allowed { cost: 1, .. }),
...
mod middleware_tests {
    ...
    fn invalid_headers_fall_through_to_direct() {
        ...
        assert_eq!(result, direct, "All invalid headers should fallback to direct IP");
    }

    /// BUG FIX VERIFICATION: Suspicious fingerprint penalty affects current request
    #[tokio::test]
    async fn suspicious_fingerprint_decreases_reputation_for_current_request() {
        ...
        assert!(matches!(decision, ProtectionDecision::Block),
            "IP with score 0 from repeated suspicious fingerprints should be blocked, got: {:?}", decision);
    }
...
mod integration_attacks {
    ...
    fn complete_legitimate_smtp_session() {
        ...
        // EHLO
        assert_eq!(prot.process_command("EHLO mail.example.com").unwrap(), SmtpAction::Continue);
        assert_eq!(prot.state(), SmtpState::GreetingReceived);

        // MAIL FROM
        assert_eq!(prot.process_command("MAIL FROM:<sender@example.com>").unwrap(), SmtpAction::Continue);

        // RCPT TO (multiple)
        assert_eq!(prot.process_command("RCPT TO:<r1@example.com>").unwrap(), SmtpAction::Continue);
        assert_eq!(prot.process_command("RCPT TO:<r2@example.com>").unwrap(), SmtpAction::Continue);
    ...
    fn bot_during_adaptive_attack() {
        ...
        let assessment = behavior.analyze();
        assert!(assessment.has_sufficient_data);
        assert!(assessment.bot_probability > 0.3,
            "Bot during attack should have elevated probability: {}", assessment.bot_probability);
...
mod invariant_tests {
    ...
    fn adaptive_threshold_bounded_under_random_input() {
        ...
        let rps_values = [0.0, 0.001, 1.0, 100.0, 10_000.0, 1_000_000.0, f64::MAX / 2.0];
        for &rps in rps_values.iter().cycle().take(100) {
            ...
            assert!(t >= 50 && t <= 10_000,
                "Threshold out of bounds: {} (min=50, max=10000)", t);
        }
    }
    ...
    fn bot_probability_always_in_unit_range() {
        ...
        let assessment = behavior.analyze();
        assert!(assessment.bot_probability >= 0.0 && assessment.bot_probability <= 1.0,
            "Bot probability out of range: {}", assessment.bot_probability);
        ...
        assert!(assessment2.bot_probability >= 0.0 && assessment2.bot_probability <= 1.0,
            "Bot probability out of range: {}", assessment2.bot_probability);
    }
