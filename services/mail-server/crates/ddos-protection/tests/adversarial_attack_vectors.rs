...
//!
//! This test file covers attack vectors not tested elsewhere:
//! 1. Protocol Fuzzing - Malformed/edge-case inputs
...
use ddos_protection::ml::{AnomalyEnsemble, EnsembleWeights, FeatureVector, IsolationForest, IsolationForestConfig, StatisticalThresholds};
...
mod timing_window_tests {
    ...
    fn test_rate_limit_window_boundary() {
        ...
        assert!(observations_at_boundary > 0, "No observations at boundaries");
    }
    ...
    fn test_simultaneous_updates() {
        ...
        assert_eq!(success_count.load(Ordering::SeqCst), 10, "Not all updates succeeded");
    }
    ...
    fn test_cold_start_race() {
        ...
        let thresholds: Vec<u64> = handles.into_iter()
...
mod state_corruption_tests {
    ...
    fn test_state_reset_consistency() {
        ...
        assert!(result.is_ok() || matches!(result, Err(_)), "State not properly reset");
    }
...
mod fingerprint_evasion_tests {
    ...
    fn test_fingerprint_rotation_attack() {
        ...
        for (i, behavior) in behaviors.iter().enumerate() {
            ...
            assert!(
                score >= 0.0 && score <= 1.0,
                "Invalid score {} for behavior {}", score, i
            );
        }
    }
    ...
    fn test_timing_jitter_evasion() {
        ...
        let assessment = behavior.analyze();
        assert!(assessment.bot_probability.is_finite(), "Score not finite with jittered timing");
    }
    ...
    fn test_human_mimicry_attack() {
        ...
        let endpoints = [
            "/", "/about", "/products", "/products/1", "/cart",
            "/api/user", "/contact", "/search?q=test", "/products/2",
        ];
        ...
        assert!(assessment.bot_probability.is_finite(), "Score should be finite for mimicry attack");
    }
}
...
mod distributed_attack_tests {
    ...
    fn test_coordinated_multi_ip_attack() {
        ...
        for ip in &attacker_ips {
            for _ in 0..5 {
                reputations.entry(*ip).or_insert_with(ReputationScore::default).record_request();
            }
        }

        // Total traffic is high even though per-IP is low
        let total_requests: u64 = reputations.iter()
    ...
    fn test_amplification_pattern() {
        ...
        let amplification_endpoints = vec![
            ("/api/export/all", RequestCost::new(50000, 10_000_000, 100, 0)),
            ("/api/report/full", RequestCost::new(100000, 50_000_000, 500, 5)),
            ("/api/search?q=*", RequestCost::new(200000, 100_000_000, 1000, 0)),
        ];
...
mod rate_limit_bypass_tests {
    ...
    fn test_xff_header_cycling() {
        ...
        for i in 0..1000u32 {
            ...
            reputations.entry(fake_ip).or_insert_with(ReputationScore::default).record_request();
        }
    ...
    fn test_dual_stack_bypass() {
        ...
        for _ in 0..50 {
            reputations.entry(ipv4).or_insert_with(ReputationScore::default).record_request();
            reputations.entry(ipv6).or_insert_with(ReputationScore::default).record_request();
        }
...
mod hash_collision_tests {
    ...
    fn test_ip_hash_collision_safety() {
        ...
        for (i, ip) in potential_collisions.iter().enumerate() {
            ...
            assert_eq!(rep.total_requests, expected, 
                "IP {} has wrong count: {} vs {}", ip, rep.total_requests, expected);
        }
    }
...
mod config_edge_case_tests {
    ...
    fn test_contradictory_config() {
        ...
        // The system correctly enforces invariants - either:
        // 1. It panics (correct defensive behavior)
        ...
        assert!(threshold > 0, "Threshold should be valid even with bad config");
    }
    ...
    fn test_max_config_values() {
        ...
        assert!(threshold >= u64::MAX / 4 || threshold <= u64::MAX / 2 || threshold == 0,
...
mod multi_vector_tests {
    ...
    fn test_exhaustion_with_evasion() {
        ...
        let patterns = vec![
            ...
            ("/api/export", "POST", false, RequestCost::new(50000, 10000000, 50, 2)),
            ("/about", "GET", false, RequestCost::new(10, 100, 0, 0)),
            ("/api/process", "POST", false, RequestCost::new(100000, 50000000, 100, 5)),
        ...
        let assessment = behavior.analyze();
        assert!(assessment.bot_probability.is_finite(), "Bot score should be calculable");
    }
}
...
mod adversarial_ml_tests {
    ...
    fn test_adversarial_feature_crafting() {
        ...
        let normal_data: Vec<[f64; 10]> = (0..100)
            .map(|i| [
                100.0 + (i as f64 * 0.1), 1000.0, 60.0, 100.0, 50.0,
                10.0, 0.5, 0.01, 10.0, 0.2,
            ])
            .collect();
        ...
        for (i, adversarial) in adversarial_inputs.iter().enumerate() {
            ...
            assert!(score.is_finite(), "Score should be finite for adversarial input {}", i);
            assert!(score >= 0.0 && score <= 1.0, "Score out of range for input {}: {}", i, score);
        }
    }
    ...
    fn test_model_poisoning_resistance() {
        ...
        for i in 0..20 {
            training_data.push([
                ...
                50000.0, 10.0, 10000.0, 5.0,
                100.0, 0.05, 0.5, 1000.0, 3.0,
            ]);
        }
        ...
        assert!(score > 0.3, "Obvious anomaly should still be detected despite poisoning: {}", score);
    }
