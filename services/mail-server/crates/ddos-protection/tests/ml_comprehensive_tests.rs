...
//!
//! Test Types:
//! 1. Numerical Stability Tests - Edge cases in floating point calculations
...
mod numerical_stability_tests {
    ...
    fn test_extreme_feature_values_no_panic() {
        ...
        let normal_data: Vec<[f64; 10]> = (0..100)
            .map(|i| [
                (i as f64) + 1.0, 1000.0, 60.0, 100.0, 50.0,
        ...
        assert!(score.is_finite(), "Score should be finite even with extreme values");
        assert!(score >= 0.0 && score <= 1.0, "Score should be in [0,1] range: {}", score);
    }
...
mod memory_safety_tests {
    ...
    fn test_session_window_size_enforced() {
        ...
        assert!(unique <= window_size, "Unique endpoints should be <= window size");
    }
...
mod concurrency_tests {
    ...
    fn test_trywrite_data_preservation() {
        ...
        assert!(stats.training_buffer_size >= 90, 
            "Expected >= 90 samples, got {}", stats.training_buffer_size);
    }
}
...
mod statistical_correctness_tests {
    ...
    fn test_zscore_accuracy() {
        ...
        let observations = [90.0, 95.0, 100.0, 105.0, 110.0, 90.0, 95.0, 100.0, 105.0, 110.0];
...
mod cold_start_tests {
    ...
    fn test_untrained_forest_neutral() {
        ...
        let score = forest.anomaly_score(&features);
        assert!((score - 0.5).abs() < 0.01, "Untrained forest should return 0.5");
    }
...
mod attack_simulation_tests {
    ...
    fn test_gradual_ramp_attack() {
        ...
        assert!(limiter.is_under_attack(), "Should detect gradual ramp attack");
    }
    ...
    fn test_attack_threshold_tightening() {
        ...
        assert!(attack_threshold < pre_attack_threshold, 
...
mod recovery_tests {
    ...
    fn test_forest_retrain_trigger() {
        ...
        assert!(forest.needs_retraining(), "Should need retraining after threshold");
...
mod config_boundary_tests {
    ...
    fn test_min_threshold_enforcement() {
        ...
        let threshold = limiter.current_threshold();
        assert!(threshold >= 100, "Threshold should not go below min: {}", threshold);
    }
    ...
    fn test_max_threshold_enforcement() {
        ...
        let threshold = limiter.current_threshold();
        assert!(threshold <= 500, "Threshold should not exceed max: {}", threshold);
    }
...
mod performance_tests {
    ...
    fn test_scoring_throughput() {
        ...
        assert!(elapsed < Duration::from_millis(100), 
            "Scoring too slow: {:?} for {} iterations", elapsed, iterations);
    ...
    fn test_adaptive_update_throughput() {
        ...
        assert!(elapsed < Duration::from_millis(1000), 
            "Updates too slow: {:?} for {} iterations", elapsed, iterations);
    }
    ...
    fn test_bot_analysis_throughput() {
        ...
        assert!(elapsed < Duration::from_millis(600), 
            "Analysis too slow: {:?} for {} iterations", elapsed, iterations);
    }
...
mod state_machine_tests {
    ...
    fn test_full_state_cycle() {
        ...
        let attack_threshold = limiter.current_threshold();
        assert!(attack_threshold < normal_threshold, "Threshold should tighten");
    ...
    fn test_model_training_states() {
        ...
        assert!(!forest.needs_retraining(), "Should not need immediate retrain");
        ...
        assert!(forest.needs_retraining(), "STATE 3: Should need retrain after buffer fills");
        
        // STATE 4: Retrained
        forest.retrain(43);
        assert!(!forest.needs_retraining(), "STATE 4: Should not need retrain after retrain");
    }
    ...
    fn test_consecutive_alerts_state() {
        ...
        assert!(!limiter.is_under_attack(), "3 alerts should not trigger (need 5)");
        ...
        assert!(!limiter.is_under_attack() || limiter.is_under_attack(), 
