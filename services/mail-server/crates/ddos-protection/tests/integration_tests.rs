...
// Pre-populate with known fingerprints
            ...
            db.known_fingerprints.insert("curl_generic".to_string(), FingerprintClass::SuspiciousBrowser);
            db.known_fingerprints.insert("googlebot_v1".to_string(), FingerprintClass::KnownBot);
            db.known_fingerprints.insert("bad_scanner_v1".to_string(), FingerprintClass::Malicious);
...
mod cost_session_tests {
    ...
    impl CostTracker {
        fn new(default_budget: i64) -> Self {
            ...
            costs.insert("/api/search".to_string(), EndpointCost {
                cpu_weight: 50,
                memory_weight: 100,
                io_weight: 20,
            });
            costs.insert("/api/login".to_string(), EndpointCost {
                cpu_weight: 100,
                memory_weight: 50,
                io_weight: 10,
            });
            costs.insert("/api/data".to_string(), EndpointCost {
                cpu_weight: 10,
                memory_weight: 10,
                io_weight: 5,
            });
...
// Get cost first before mutably borrowing session_budgets
            ...
            let budget = self.session_budgets
        ...
        fn remaining_budget(&self, session: &str) -> i64 {
            *self.session_budgets.get(session).unwrap_or(&self.default_budget)
        }
    }
