...
mod slowloris_tests {
    ...
    impl ConnectionTracker {
        ...
        fn can_accept_connection(&self, ip: &str) -> bool {
            let count = self.connections.keys()
        ...
        fn add_connection(&mut self, conn_id: &str) {
            self.connections.insert(conn_id.to_string(), ConnectionState {
...
mod app_layer_tests {
    ...
    impl PatternDetector {
        ...
        fn detect_credential_stuffing(&self, ip: &str) -> bool {
            ...
            let request_count = self.endpoint_patterns
                .get(&key)
                .map(|v| v.len())
                .unwrap_or(0);
...
mod distributed_attack_tests {
    ...
    impl GlobalRateTracker {
        ...
        fn top_offenders(&self, n: usize) -> Vec<(String, u64)> {
            let mut sorted: Vec<_> = self.ip_requests.iter()
                .map(|(k, v)| (k.clone(), *v))
                .collect();
...
mod adaptive_challenge_tests {
    ...
    fn test_js_challenge_medium_threat() {
        ...
        assert_eq!(challenger.get_challenge("some_ip"), ChallengeType::JavaScript);
    }
    ...
    fn test_pow_challenge_high_threat() {
        ...
        assert_eq!(challenger.get_challenge("suspicious_ip"), ChallengeType::ProofOfWork);
    }
