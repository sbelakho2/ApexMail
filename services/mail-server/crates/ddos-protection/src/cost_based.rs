...
impl RequestCost {
    ...
    pub fn new(cpu_us: u32, memory_bytes: u32, io_ops: u32, external_calls: u32) -> Self {
        Self { cpu_us, memory_bytes, io_ops, external_calls }
    }
...
pub static ENDPOINT_COSTS: Lazy<HashMap<&'static str, RequestCost>> = Lazy::new(|| {
    ...
    // High-cost endpoints
    m.insert("/v1/messages/send", RequestCost::new(5000, 1024 * 1024, 5, 1));
    m.insert("/v1/messages/send/batch", RequestCost::new(50000, 10 * 1024 * 1024, 50, 10));
    m.insert("/v1/templates/render", RequestCost::new(10000, 512 * 1024, 2, 0));
...
impl CostBasedLimiter {
    ...
    pub fn check(&self, tenant_id: &str, endpoint: &str, override_cost: Option<RequestCost>) -> CostDecision {
        ...
        // Check tenant budget
        let budget_entry = self.tenant_budgets
