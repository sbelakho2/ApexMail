...
pub fn process_bundle(bundle: &StixBundle) -> Vec<ProcessedIndicator> {
    ...
    for obj in &bundle.objects {
        if let StixObject::Indicator(ind) = obj {
            let extracted = ind.pattern.as_deref().map(extract_indicators).unwrap_or_default();
...
mod tests {
    ...
    fn test_extract_ipv4_indicator() {
        ...
        assert_eq!(indicators[0], ExtractedIndicator::Ipv4("192.168.1.1".into()));
    }
    ...
    fn test_extract_domain_indicator() {
        ...
        let indicators = extract_indicators(pattern);
        assert_eq!(indicators.len(), 1);
        assert_eq!(indicators[0], ExtractedIndicator::Domain("evil.example.com".into()));
    }
    ...
    fn test_extract_url_indicator() {
        ...
        let indicators = extract_indicators(pattern);
        assert_eq!(indicators.len(), 1);
        assert_eq!(indicators[0], ExtractedIndicator::Url("http://evil.com/malware.exe".into()));
    }
    ...
    fn test_extract_email_indicator() {
        ...
        let indicators = extract_indicators(pattern);
        assert_eq!(indicators.len(), 1);
        assert_eq!(indicators[0], ExtractedIndicator::Email("phisher@evil.com".into()));
    }
    ...
    fn test_parse_stix_bundle() {
        ...
        assert_eq!(indicators[0].indicator, ExtractedIndicator::Ipv4("203.0.113.5".into()));
    }
