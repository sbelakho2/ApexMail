...
fn tool_permission(tool: &str) -> &[Role] {
    match tool {
        ...
        "calculate_overage" | "calculate_payg" | "get_price_diff" | "compare_plans"
        | "get_plan_details" => &[Role::Owner, Role::Admin, Role::Developer, Role::Viewer],
        // DNS — all roles
        ...
        "generate_incident_timeline" | "get_audit_log" | "get_security_events" => {
        ...
        | "get_ip_warmup_status" => &[Role::Owner, Role::Admin],
        // Default: owner only
        _ => &[Role::Owner],
    }
}
...
pub async fn execute_tool_with_authoritative_data(
    ...
    match store.records_for_domain(&caller.tenant_id, domain).await {
        Ok(records) => serde_json::to_value(records).unwrap_or_else(
            |_| serde_json::json!({"error": "could not serialize authoritative DNS records"}),
        ),
...
fn get_plan_details(params: &serde_json::Value) -> serde_json::Value {
    ...
    details
        .get(n)
        .cloned()
        .unwrap_or(serde_json::json!({"error":format!("unknown plan: {n}")}))
}
...
fn map_provider_events(params: &serde_json::Value) -> serde_json::Value {
    ...
    let mapping = match from {
        "sendgrid" => {
            serde_json::json!({"processed":"queued","deferred":"deferred","delivered":"delivered","open":"opened","click":"clicked","bounce":"bounced","dropped":"bounced(permanent)","spamreport":"complained","group_unsubscribe":"unsubscribed","group_resubscribe":"resubscribed"})
        }
        "mailgun" => {
            serde_json::json!({"accepted":"queued","delivered":"delivered","opened":"opened","clicked":"clicked","complained":"complained","unsubscribed":"unsubscribed","failed":"bounced(permanent)","temporary_fail":"deferred"})
        }
        "mailchimp" => {
            serde_json::json!({"send":"queued","open":"opened","click":"clicked","bounce":"bounced(permanent)","soft_bounce":"deferred","abuse":"complained","unsub":"unsubscribed"})
        }
...
fn get_compliance_info(params: &serde_json::Value) -> serde_json::Value {
    ...
    match topic {
        ...
        "data_residency" => {
            serde_json::json!({"location":"EU/EEA","primary":"Helsinki, Finland","standby":"Nuremberg, Germany","encryption":"AES-256-GCM at rest, TLS 1.2+ in transit","certifications":["ISO 27001","SOC2 Type II"]})
        }
        _ => {
            serde_json::json!({"error": format!("unknown topic: {topic}. Try: gdpr_breach_notification, soc2_incident, gdpr_dsar, data_residency")})
        }
    }
}
...
fn get_ab_test_guidance(params: &serde_json::Value) -> serde_json::Value {
    ...
    let days = if list_size > 100000 {
        2
    } else if list_size > 10000 {
        4
    } else {
        7
    };
...
fn get_retry_guidance(params: &serde_json::Value) -> serde_json::Value {
    ...
    let guidance = match from {
        "sendgrid" => {
            serde_json::json!({"from":"437 on SendGrid","replace_with":"429+Retry-After header on ApexMail","backoff":"Exponential: 1s, 2s, 4s, 8s","max_retries":3,"idempotency":"Use Idempotency-Key header to prevent duplicates"})
        }
        "mailgun" => {
            serde_json::json!({"from":"429 on Mailgun","replace_with":"Same — 429+Retry-After on ApexMail","backoff":"Exponential from Retry-After value","max_retries":3})
        }
        _ => {
            serde_json::json!({"generic":"Exponential backoff from Retry-After header, max 3 retries, use Idempotency-Key header"})
        }
    };
...
fn get_bounce_classification(params: &serde_json::Value) -> serde_json::Value {
    ...
    let classifications = [
        (
            ...
            "Fix: Reduce volume, improve engagement, check reputation",
        ),
        (
            "550-5.1.1",
            "Invalid recipient",
            "Permanent",
            "Mailbox not found",
            "Fix: Remove from list immediately, do not retry",
        ),
        (
            "552-5.2.2",
            "Mailbox full",
        ...
        (
            ...
            "Fix: Check content, DNS, reputation. Suppress recipient.",
        ),
        (
            "452-4.2.2",
            "Mailbox full",
            "Temporary",
            "Recipient over quota (different code)",
            "Fix: Retry in 24-48 hours",
        ),
    ];
    let found = classifications
        .iter()
        .find(|(c, _, _, _, _)| code.contains(c) || c.contains(code));
    match found {
        Some((code, label, ptype, desc, fix)) => {
            serde_json::json!({"code":code,"label":label,"type":ptype,"description":desc,"fix":fix})
        }
        None => {
            serde_json::json!({"code":code,"type":"unknown","action":"Check RFC 3463/5248 for standard bounce codes"})
        }
    }
}
...
mod tests {
    ...
    // Existing tool tests
    #[test]
    fn test_overage_pro_160k() {
        let r = calculate_overage(&serde_json::json!({"plan":"pro","emails_sent":160000}));
        assert_eq!(r["total"], 69.0);
    }
    #[test]
    fn test_overage_starter_210k() {
        let r = calculate_overage(&serde_json::json!({"plan":"starter","emails_sent":210000}));
        assert_eq!(r["total"], 89.0);
    }
    #[test]
    fn test_payg_50k() {
    ...
    #[test]
    fn test_price_diff() {
        let r = get_price_diff(&serde_json::json!({"plan_a":"scale","plan_b":"enterprise"}));
        assert_eq!(r["diff"], 2650);
    }

    // Tenant and RBAC isolation tests
    #[test]
    fn test_tenant_isolation_blocks_cross_tenant() {
        let call = ToolCall {
    ...
    #[test]
    fn test_tenant_isolation_allows_same_tenant() {
        let call = ToolCall {
            tool: "get_audit_log".into(),
            params: serde_json::json!({}),
            tenant_id: Some("tenant-A".into()),
    ...
    #[test]
    fn test_rbac_blocks_viewer_from_billing() {
        let call = ToolCall {
            tool: "get_billing_history".into(),
            params: serde_json::json!({}),
    ...
    #[test]
    fn test_rbac_allows_owner_to_billing() {
        let call = ToolCall {
            tool: "get_billing_history".into(),
            params: serde_json::json!({}),
            tenant_id: Some("t1".into()),
    ...
    #[test]
    fn test_global_tools_skip_isolation() {
        let call = ToolCall {
            tool: "calculate_overage".into(),
            params: serde_json::json!({"plan":"pro","emails_sent":100000}),
            tenant_id: None,
            role: None,
        };
    ...
    #[test]
    fn test_warmup() {
        let r = get_warmup_schedule(&serde_json::json!({"ip_count":3}));
        assert_eq!(r["ip_count"], 3);
        assert_eq!(r["schedule"].as_array().unwrap().len(), 8);
    }
    #[test]
    fn test_blocklist() {
        let r = get_blocklist_status(&serde_json::json!({}));
        assert!(r["checks"].as_array().unwrap().len() >= 3);
    }
    #[test]
    fn test_event_map() {
        let r = map_provider_events(&serde_json::json!({"from_provider":"sendgrid"}));
        assert_eq!(r["event_mapping"]["open"], "opened");
    }
    #[test]
    fn test_compliance() {
        let r = get_compliance_info(&serde_json::json!({"topic":"gdpr_breach_notification"}));
        assert_eq!(r["deadline"], "72 hours from awareness");
    }
    #[test]
    fn test_incident_timeline() {
        let r =
            generate_incident_timeline(&serde_json::json!({"key_id":"key123","exposure_hours":4}));
        assert_eq!(r["exposure_hours"], 4);
    }
    #[test]
    fn test_recovery_plan() {
        let r = get_deliverability_recovery_plan(
            &serde_json::json!({"current_volume":2100000,"spam_rate":0.35}),
        );
        assert_eq!(r["severity"], "critical");
    }
    #[test]
    fn test_ab_guidance() {
        let r = get_ab_test_guidance(&serde_json::json!({"list_size":50000,"variants":3}));
        assert_eq!(r["variants"], 3);
    }
    #[test]
    fn test_sto() {
        let r = get_sto_info(&serde_json::json!({"timezone_count":12}));
        assert_eq!(r["timezones_served"], 12);
    }
    #[test]
    fn test_retry() {
        let r = get_retry_guidance(&serde_json::json!({"from_provider":"sendgrid"}));
        assert!(r["guidance"]["from"].as_str().unwrap().contains("SendGrid"));
    }
    #[test]
    fn test_dmarc() {
        let r = get_dmarc_analysis(&serde_json::json!({"domain":"example.com","policy":"reject"}));
        assert_eq!(r["current_policy"], "reject");
    }
    #[test]
    fn test_bounce() {
        let r = get_bounce_classification(&serde_json::json!({"bounce_code":"421-4.7.28"}));
        assert_eq!(r["label"], "Google rate limit");
    }
    #[test]
