...
async fn main() -> Result<()> {
    ...
    let enable_starttls = args
        .enable_starttls
        .unwrap_or_else(|| args.tls_cert_path.is_some() && args.tls_key_path.is_some());

    let tls_acceptor = if enable_starttls {
        let cert_path = args
            .tls_cert_path
            .clone()
            .ok_or_else(|| anyhow!("TLS_CERT_PATH is required when STARTTLS is enabled"))?;
        let key_path = args
            .tls_key_path
            .clone()
            .ok_or_else(|| anyhow!("TLS_KEY_PATH is required when STARTTLS is enabled"))?;
