...
pub fn spam_rules() -> RuleSet {
    let rules = vec![
        ...
        Rule::new("expires soon", "spam:urgency:expires", RuleCategory::Spam, Severity::Low, 2),
        ...
        Rule::new("earn extra cash", "spam:financial:cash", RuleCategory::Spam, Severity::Medium, 4),
        Rule::new("make money fast", "spam:financial:money", RuleCategory::Spam, Severity::High, 6),
        Rule::new("double your income", "spam:financial:income", RuleCategory::Spam, Severity::High, 6),
        ...
        Rule::new("unusual activity", "phishing:unusual_activity", RuleCategory::Phishing, Severity::High, 6),
        Rule::new("reset your password", "phishing:reset_password", RuleCategory::Phishing, Severity::Medium, 4),
        Rule::new("click here to verify", "phishing:click_verify", RuleCategory::Phishing, Severity::Critical, 9),
        ...
        Rule::new("free trial", "policy:free_trial", RuleCategory::ContentPolicy, Severity::Low, 1),
    ];
    RuleSet::new(rules)
}
...
pub fn phishing_rules() -> RuleSet {
    let rules = vec![
        ...
        Rule::new("suspended account", "phishing:suspended", RuleCategory::Phishing, Severity::High, 7),
        Rule::new("unauthorized login", "phishing:unauthorized", RuleCategory::Phishing, Severity::High, 6),
        Rule::new("update your payment", "phishing:update_payment", RuleCategory::Phishing, Severity::Critical, 9),
        Rule::new("your account will be closed", "phishing:account_closed", RuleCategory::Phishing, Severity::Critical, 9),
        Rule::new("verify your identity immediately", "phishing:verify_urgent", RuleCategory::Phishing, Severity::Critical, 10),
        Rule::new("you must respond within", "phishing:respond_deadline", RuleCategory::Phishing, Severity::High, 7),
    ];
    RuleSet::new(rules)
}
...
mod tests {
    ...
    fn test_spam_rules_count() {
        let rs = spam_rules();
        assert!(rs.rule_count() >= 30, "Expected 30+ rules, got {}", rs.rule_count());
    }

    #[test]
    fn test_spam_rules_detect_financial() {
        let rs = spam_rules();
        let matches = rs.evaluate("Congratulations you won a million dollars! Claim your prize now!");
    ...
    fn test_spam_rules_detect_phishing() {
        let rs = spam_rules();
        let matches = rs.evaluate("Your account has been compromised. Click here to verify your identity.");
    ...
    fn test_spam_rules_clean_text() {
        let rs = spam_rules();
        let matches = rs.evaluate("Hi John, here's the quarterly report you requested. Best regards.");
    ...
    fn test_phishing_rules_has_critical() {
        let rs = phishing_rules();
        let matches = rs.evaluate("Your account has been compromised. Update your payment information immediately.");
        assert!(
    ...
    fn test_phishing_rules_score() {
        let rs = phishing_rules();
        let matches = rs.evaluate("Verify your account or your account will be closed. Click here to verify now.");
        let total: f64 = matches.iter().map(|m| m.score).sum();
        assert!(total >= 16.0, "Multiple phishing patterns should yield high score, got {total}");
    }
}
