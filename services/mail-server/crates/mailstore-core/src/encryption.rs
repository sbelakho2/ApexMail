...
pub fn decrypt_with_aad(data: &[u8], master_key: &[u8], aad: &[u8]) -> Result<Vec<u8>> {
    ...
    // Detect envelope version. v1 is `version || nonce || ciphertext+tag`.
    // Anything else is treated as legacy (`nonce || ciphertext+tag`) but
    // only when AAD is empty — versioned bindings cannot be recovered from
    // an unversioned ciphertext.
    let (version, nonce_bytes, ciphertext) = if !data.is_empty() && data[0] == ENVELOPE_VERSION_V1 {
