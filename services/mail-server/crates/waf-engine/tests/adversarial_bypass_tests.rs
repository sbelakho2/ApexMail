...
mod xss_evasion {
    ...
    fn event_handler_variants() {
        ...
        let handlers = vec!["onload", "onerror", "onclick", "onmouseover", "onfocus"];
    ...
    fn svg_tags() {
        ...
        let payloads = vec!["<svg onload=alert(1)>", "<svg/onload=alert(1)>"];

        for payload in payloads {
            let result = fast_path_check(payload);
            assert!(result.has_xss_patterns, "SVG XSS not detected: {}", payload);
        }
    }
    ...
    fn broken_tags_still_dangerous() {
        ...
        let payloads = vec!["<script>alert(1)", "</script><script>alert(1)</script>"];
...
mod graphql_evasion {
    ...
    fn fragment_handling() {
        ...
        assert!(
            result.looks_like_graphql,
            "Failed to parse query with fragments"
        );
    }
}
...
mod polymorphic_attacks {
    ...
    fn mixed_sqli_and_xss() {
        ...
        assert!(result.has_xss_patterns, "XSS not detected in mixed payload");
    }
    ...
    fn polyglot_payloads() {
        ...
        assert!(result.has_xss_patterns, "Polyglot XSS not detected");
    }
}
...
mod stress_tests {
    ...
    fn alternating_safe_and_dangerous() {
        ...
        let payload =
            "normal text SELECT * FROM users normal text <script>alert(1)</script> more normal";
        let result = fast_path_check(payload);
