...
mod json_edge_cases {
    ...
    fn test_deeply_nested_json() {
        ...
        assert!(!result.parsed_ok);
        assert!(result.error.as_ref().map(|e| e.contains("depth")).unwrap_or(false));
    }
    ...
    fn test_mixed_types() {
        ...
        let values: Vec<&str> = result.string_values.iter().map(|v| v.value.as_str()).collect();
...
mod graphql_edge_cases {
    ...
    fn test_string_arg_injection() {
        ...
        let result = extract_graphql_values(query);
        assert!(result.looks_like_graphql);
        let values: Vec<&str> = result.string_arguments.iter().map(|v| v.value.as_str()).collect();
    ...
    #[test]
    fn test_mutation() {
        let query = r#"mutation { createUser(input: {name: "test", email: "test@test.com"}) { id } }"#;
        let result = extract_graphql_values(query);
        assert!(result.looks_like_graphql);
    }
