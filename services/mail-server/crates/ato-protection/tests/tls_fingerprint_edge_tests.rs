...
use ato_protection::tls_fingerprint::{EnhancedDeviceFingerprint, TlsFingerprint, UserTlsHistory};
...
mod fingerprint_edge_cases {
    ...
    fn test_max_value_cipher() {
        let fp = TlsFingerprint::from_client_hello(0x0303, &[0xFFFF], &[], &[], &[]);
...
mod user_history_edge_cases {
    ...
    fn test_many_different_fingerprints() {
        ...
        let new_fp = TlsFingerprint::from_client_hello(0x0304, &[0xFFFF], &[], &[], &[]);
    ...
    fn test_repeated_same_fingerprint() {
        let mut history = UserTlsHistory::new(100);
        let fp = TlsFingerprint::from_client_hello(0x0303, &[0x1301], &[], &[], &[]);
...
mod enhanced_fingerprint_edge_cases {
    ...
    fn test_enhanced_fingerprint_creation() {
        let tls_fp = TlsFingerprint::from_client_hello(0x0303, &[0x1301], &[], &[], &[]);
        let enhanced = EnhancedDeviceFingerprint::new("Mozilla/5.0", "192.168.1.100", Some(tls_fp));
    ...
    fn test_empty_user_agent() {
        let tls_fp = TlsFingerprint::from_client_hello(0x0303, &[0x1301], &[], &[], &[]);
        let enhanced = EnhancedDeviceFingerprint::new("", "10.0.0.1", Some(tls_fp));
        assert!(!enhanced.hash.is_empty());
    }
    ...
    fn test_long_user_agent() {
        ...
        let enhanced = EnhancedDeviceFingerprint::new(&long_ua, "10.0.0.1", Some(tls_fp));
    ...
    fn test_no_tls_fingerprint() {
        let enhanced = EnhancedDeviceFingerprint::new("Mozilla/5.0", "192.168.1.1", None);
        assert!(!enhanced.hash.is_empty());
