...
impl TlsFingerprint {
    ...
    pub fn from_client_hello(
        ...
        let mut sorted_ciphers: Vec<String> =
            cipher_suites.iter().map(|c| format!("{:04x}", c)).collect();
        sorted_ciphers.sort();

        let mut sorted_exts: Vec<String> =
            extensions.iter().map(|e| format!("{:04x}", e)).collect();
        sorted_exts.sort();
...
mod tests {
    ...
    fn test_fingerprint_risk() {
        ...
        let bot = TlsFingerprint::from_client_hello(0x0301, &[0x002f], &[], &[], &[]);
    ...
    fn test_enhanced_device_fingerprint() {
        ...
        let device =
            EnhancedDeviceFingerprint::new("Mozilla/5.0 Chrome/120", "192.168.1.100", Some(tls));
