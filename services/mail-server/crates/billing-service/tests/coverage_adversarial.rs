...
// ---------------------------------------------------------------------------
// hetzner_ip_provider.rs — local HTTP mock, typed errors, no partial state
// ---------------------------------------------------------------------------

use billing_service::hetzner_ip_provider::{HetznerIpProvider, IpStatus, ProvisionError};
...
struct MockScript {
    responses: StdArc<StdMutex<HashMap<String, (u16, String)>>>,
    calls: StdArc<StdMutex<Vec<MockCall>>>,
}
...
async fn mock_handler(
    axum::extract::State(script): axum::extract::State<MockScript>,
    ...
    let (status, response_body) = script
        ...
        .get(&path)
        .cloned()
        .unwrap_or_else(|| (404, "{\"error\":\"unscripted\"}".to_string()));
    script.calls.lock().expect("mock lock").push(MockCall {
        method,
        path,
        authorization,
        body,
...
async fn spawn_mock(script: MockScript) -> String {
...
fn mock_provider(h: &Harness, base: &str, mta_server_id: Option<u64>) -> HetznerIpProvider {
    HetznerIpProvider::new(
        "hcloud-test-token".to_string(),
        h.pool.clone(),
        "fsn1".to_string(),
        mta_server_id,
    )
...
db_test!(hetzner_provision_success_records_ip_and_queue, |h| {
    let tenant = "cov_hetzner_ok";
    seed_tenant(&h, tenant, "free").await;
    let script = MockScript::default();
    script.route(
        "/v1/floating_ips",
        201,
        r#"{"floating_ip":{"id":4242,"ip":"203.0.113.7"}}"#,
    );
    script.route("/v1/floating_ips/4242/actions/assign", 200, "{}");
    script.route("/v1/floating_ips/4242/actions/change_dns_ptr", 200, "{}");
    let base = spawn_mock(script.clone()).await;

    let provider = mock_provider(&h, &base, Some(77));
    let ip = provider
        .provision_ip(tenant, Some("example.test"))
        .await
        .expect("provision");
    assert_eq!(ip.ip_address, "203.0.113.7");
    assert_eq!(ip.hetzner_floating_ip_id, Some(4242));
    assert_eq!(ip.status, IpStatus::Warming);
    assert_eq!(ip.rdns_hostname.as_deref(), Some("mail.example.test"));
    assert_eq!(ip.assigned_server_id, Some(77));

    let calls = script.calls();
    assert_eq!(calls.len(), 3, "create + assign + rDNS");
    assert_eq!(calls[0].method, "POST");
    assert_eq!(calls[0].path, "/v1/floating_ips");
    assert_eq!(
        calls[0].authorization.as_deref(),
        Some("Bearer hcloud-test-token"),
        "the API token is sent as a bearer credential"
    );
    let create_body: serde_json::Value =
        serde_json::from_slice(&calls[0].body).expect("create body json");
    assert_eq!(create_body["type"], "ipv4");
    assert_eq!(create_body["home_location"], "fsn1");
    assert_eq!(create_body["labels"]["tenant_id"], tenant);
    assert_eq!(create_body["labels"]["service"], "apexmail");

    let (status, floating_id, server_id, warmup_day, rdns): (
        String,
        Option<i64>,
        Option<i64>,
        i32,
        ...
        "SELECT status, hetzner_floating_ip_id, hetzner_server_id, warmup_day, rdns_hostname
         FROM dedicated_ips WHERE tenant_id = $1",
    )
    .bind(tenant)
    .fetch_one(&h.pool)
    .await
    ...
    assert_eq!(floating_id, Some(4242));
    assert_eq!(server_id, Some(77));
    assert_eq!(warmup_day, 0);
    assert_eq!(rdns.as_deref(), Some("mail.example.test"));

    let queue: (String, Option<String>) = sqlx::query_as(
        "SELECT status, error_message FROM ip_provisioning_queue WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 1",
    )
    .bind(tenant)
    .fetch_one(&h.pool)
    .await
    .expect("queue row");
    assert_eq!(queue.0, "completed");
    assert!(queue.1.is_none());
});

db_test!(hetzner_provision_api_error_records_failure_and_no_ip, |h| {
    let tenant = "cov_hetzner_500";
    seed_tenant(&h, tenant, "free").await;
    let script = MockScript::default();
    script.route("/v1/floating_ips", 500, "upstream exploded");
    let base = spawn_mock(script.clone()).await;

    let provider = mock_provider(&h, &base, None);
    let error = provider
        .provision_ip(tenant, None)
        .await
        .expect_err("500 must be a typed error");
    assert!(matches!(error, ProvisionError::HetznerApi(_)));
    assert!(error.to_string().contains("upstream exploded"));

    let rows: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM dedicated_ips WHERE tenant_id = $1")
        .bind(tenant)
        .fetch_one(&h.pool)
        .await
        .expect("count ips");
    assert_eq!(rows, 0, "no partial IP state on API failure");
    let (status, error_message): (String, Option<String>) = sqlx::query_as(
        "SELECT status, error_message FROM ip_provisioning_queue WHERE tenant_id = $1
    ...
    assert_eq!(error_message.as_deref(), Some("upstream exploded"));
});

db_test!(hetzner_provision_malformed_json_is_typed_error, |h| {
    let tenant = "cov_hetzner_json";
    seed_tenant(&h, tenant, "free").await;
    let script = MockScript::default();
    script.route("/v1/floating_ips", 200, "this is not json");
    let base = spawn_mock(script.clone()).await;
    ...
    assert!(matches!(error, ProvisionError::HetznerApi(_)), "{error:?}");
    let rows: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM dedicated_ips WHERE tenant_id = $1")
        .bind(tenant)
        .fetch_one(&h.pool)
        .await
        .expect("count ips");
    assert_eq!(rows, 0, "malformed response must not create a row");
});

db_test!(hetzner_rdns_failure_does_not_abort_provision, |h| {
    let tenant = "cov_hetzner_rdns";
    seed_tenant(&h, tenant, "free").await;
    let script = MockScript::default();
    script.route(
        "/v1/floating_ips",
        201,
        r#"{"floating_ip":{"id":9,"ip":"203.0.113.9"}}"#,
    );
    script.route(
        "/v1/floating_ips/9/actions/change_dns_ptr",
        429,
        "rate limited",
    );
    let base = spawn_mock(script.clone()).await;
    ...
    let provider = mock_provider(&h, &base, None);
    let ip = provider
        .provision_ip(tenant, Some("rdns.test"))
        .await
        .expect("rDNS failure is tolerated");
    assert!(
        ip.rdns_hostname.is_none(),
        "unset rDNS is recorded as retryable"
    );
    let stored: Option<String> =
        sqlx::query_scalar("SELECT rdns_hostname FROM dedicated_ips WHERE tenant_id = $1")
            .bind(tenant)
            .fetch_one(&h.pool)
            .await
            .expect("stored rdns");
...
db_test!(hetzner_release_unknown_ip_is_not_found_without_http, |h| {
    let script = MockScript::default();
    let base = spawn_mock(script.clone()).await;
    let provider = mock_provider(&h, &base, None);

    let error = provider
        .release_ip("does-not-exist")
        .await
        .expect_err("unknown IP");
    assert!(matches!(error, ProvisionError::NotFound(_)));
    assert!(script.calls().is_empty(), "no HTTP call for an unknown IP");
});

db_test!(
    hetzner_release_handles_404_and_5xx_without_partial_state,
    |h| {
        let tenant = "cov_hetzner_rel";
        seed_tenant(&h, tenant, "free").await;
        seed_released_ip(&h, "cov_ip_rel_1", tenant, "203.0.113.11", 111).await;

        // 5xx: the release fails and the row keeps its state.
        let failing = MockScript::default();
        failing.route("/v1/floating_ips/111", 500, "boom");
        let failure_base = spawn_mock(failing.clone()).await;
        let provider = mock_provider(&h, &failure_base, None);
        let error = provider
            .release_ip("cov_ip_rel_1")
            .await
            .expect_err("5xx must fail the release");
        assert!(matches!(error, ProvisionError::HetznerApi(_)), "{error:?}");
        let status: String =
            sqlx::query_scalar("SELECT status FROM dedicated_ips WHERE id = 'cov_ip_rel_1'")
                .fetch_one(&h.pool)
                .await
                .expect("status");
        assert_eq!(
            status, "warming",
            "failed release must not flip local state"
        );

        // 404: already gone upstream is an idempotent success.
        let gone = MockScript::default();
        gone.route("/v1/floating_ips/111", 404, "not found");
        let gone_base = spawn_mock(gone.clone()).await;
        let provider = mock_provider(&h, &gone_base, None);
        provider
            .release_ip("cov_ip_rel_1")
            .await
        ...
        assert_ne!(status, "warming", "released IP leaves the active set");
    }
);

async fn seed_released_ip(h: &Harness, id: &str, tenant: &str, ip: &str, hetzner_id: i64) {
    sqlx::query(
        "INSERT INTO dedicated_ips (id, tenant_id, ip_address, status, warmup_day,
                                    hetzner_floating_ip_id, created_at, allocated_at,
                                    warmup_started_at)
         VALUES ($1, $2, $3, 'warming', 0, $4, NOW(), NOW(), NOW())",
    )
    .bind(id)
    .bind(tenant)
    .bind(ip)
...
db_test!(hetzner_list_and_warmup_graduation, |h| {
    let tenant = "cov_hetzner_list";
    seed_tenant(&h, tenant, "free").await;
    seed_released_ip(&h, "cov_ip_list_1", tenant, "203.0.113.21", 21).await;
    sqlx::query(
        "INSERT INTO dedicated_ips (id, tenant_id, ip_address, status, warmup_day,
                                    created_at, allocated_at, warmup_started_at)
         VALUES ('cov_ip_list_2', $1, '203.0.113.22', 'active', 30, NOW(), NOW(), NOW())",
    )
    .bind(tenant)
    .execute(&h.pool)
    .await
    .expect("seed active ip");
    // Fully warmed row (allocated 100 days ago) must graduate to active.
    sqlx::query(
        "INSERT INTO dedicated_ips (id, tenant_id, ip_address, status, warmup_day,
                                    created_at, allocated_at, warmup_started_at)
         VALUES ('cov_ip_list_3', $1, '203.0.113.23', 'warming', 0,
                 NOW() - INTERVAL '100 days', NOW() - INTERVAL '100 days', NOW() - INTERVAL '100 days')",
    )
    .bind(tenant)
    .execute(&h.pool)
    .await
    .expect("seed old warming ip");

    let script = MockScript::default();
    let base = spawn_mock(script).await;
    let provider = mock_provider(&h, &base, None);

    let ips = provider.list_tenant_ips(tenant).await.expect("list ips");
    assert_eq!(ips.len(), 3);
    assert!(ips.iter().any(|ip| ip.status == IpStatus::Active));
    assert!(ips.iter().any(|ip| ip.status == IpStatus::Warming));

    let updated = provider.update_warmup_days().await.expect("warmup update");
    assert!(updated >= 2, "both warming rows were touched: {updated}");
    let graduated: String =
        sqlx::query_scalar("SELECT status FROM dedicated_ips WHERE id = 'cov_ip_list_3'")
            .fetch_one(&h.pool)
            .await
            .expect("graduated row");
    assert_eq!(graduated, "active", "full warmup graduates to active");
    let warmup_day: i32 =
        sqlx::query_scalar("SELECT warmup_day FROM dedicated_ips WHERE id = 'cov_ip_list_3'")
            .fetch_one(&h.pool)
            .await
            .expect("warmup day");
    assert!(
        warmup_day >= 99,
        "warmup day tracks elapsed days: {warmup_day}"
    );
});

db_test!(hetzner_process_pending_requests_isolates_failures, |h| {
    let ok = "cov_hetzner_queue_a";
    let clash = "cov_hetzner_queue_b";
    seed_tenant(&h, ok, "free").await;
    seed_tenant(&h, clash, "free").await;

    // Both tenants' provisioning calls return the SAME IP address; the
    // partial unique index on active ip_address makes the second insert
    // fail AFTER its HTTP call, exercising per-request isolation.
    let script = MockScript::default();
    script.route(
        "/v1/floating_ips",
        201,
        r#"{"floating_ip":{"id":777,"ip":"203.0.113.77"}}"#,
    );
    let base = spawn_mock(script.clone()).await;

    sqlx::query(
        "INSERT INTO ip_provisioning_queue (id, tenant_id, request_type, region, quantity, status, created_at)
         VALUES (gen_random_uuid(), $1, 'hetzner_ip', 'us-east-1', 1, 'pending', NOW() - INTERVAL '2 minutes'),
                (gen_random_uuid(), $2, 'hetzner_ip', 'us-east-1', 1, 'pending', NOW() - INTERVAL '1 minute')",
    )
    .bind(ok)
    .bind(clash)
    .execute(&h.pool)
    ...
    assert_eq!(
        processed, 1,
        "one success is counted, one failure is isolated"
    );
    let ips: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM dedicated_ips WHERE ip_address = '203.0.113.77'")
            .fetch_one(&h.pool)
            .await
            .expect("count ips");
    assert_eq!(ips, 1, "no duplicate active IP on the clash");
});
...
// ---------------------------------------------------------------------------
// hetzner_ip_provider.rs — status mapping and env configuration
// ---------------------------------------------------------------------------

db_test!(
    hetzner_list_maps_every_status_and_env_config_is_optional,
    |h| {
        let tenant = "cov_hetzner_statuses";
        seed_tenant(&h, tenant, "free").await;
        // Every state the schema CHECK admits (migration 207). The mapper
        // must be TOTAL over this vocabulary: each value maps to its
        // canonical IpStatus variant — never a silent fail-closed guess.
        // Released/retired rows are excluded from the active set.
        for (index, (id_suffix, status)) in [
            ("pending", "pending"),
            ("prov", "provisioning"),
            ("created", "created"),
            ("attached", "attached"),
        ...
        .enumerate()
        {
            sqlx::query(
                "INSERT INTO dedicated_ips (id, tenant_id, ip_address, status, warmup_day,
                                            allocated_at, warmup_started_at, created_at)
                 VALUES ($1, $2, $3, $4, 0, NOW(), NOW(), NOW())",
            )
            .bind(format!("cov_status_{id_suffix}"))
            .bind(tenant)
            .bind(format!("203.0.113.{}", 100 + index))
            .bind(status)
            .execute(&h.pool)
            .await
            .expect("dedicated ip");
        ...
        let provider = mock_provider(&h, "http://127.0.0.1:9/v1", None);
        let ips = provider.list_tenant_ips(tenant).await.expect("list");
        let by_id = |id: &str| {
            ips.iter()
                .find(|ip| ip.id == id)
        ...
        use billing_service::hetzner_ip_provider::IpStatus;
        for (id, expected) in [
            ("cov_status_pending", IpStatus::Pending),
            ("cov_status_prov", IpStatus::Provisioning),
            ("cov_status_created", IpStatus::Created),
            ("cov_status_attached", IpStatus::Attached),
            ("cov_status_rdns", IpStatus::RdnsReady),
            ("cov_status_warming", IpStatus::Warming),
            ("cov_status_active", IpStatus::Active),
            ("cov_status_suspended", IpStatus::Suspended),
            ("cov_status_cooldown", IpStatus::Cooldown),
            ("cov_status_failed", IpStatus::Failed),
            ("cov_status_cleanup", IpStatus::CleanupFailed),
            ("cov_status_releasing", IpStatus::Releasing),
        ] {
            assert_eq!(by_id(id), expected, "{id} must map honestly");
        }
        assert_eq!(ips.len(), 12, "retired rows leave the active set");
        assert!(
            !ips.iter().any(|ip| ip.id == "cov_status_retired"),
            "retired rows are not listed"
        );

        // The vocabulary the mapper accepts IS the CHECK's vocabulary. The
        // old dead arms (`degraded`/`disabled`/`released`) are values the
        // schema cannot hold — the database itself refuses them.
        for impossible in ["degraded", "disabled", "released"] {
            let rejected = sqlx::query(
                "INSERT INTO dedicated_ips (id, tenant_id, ip_address, status, warmup_day,
                                            allocated_at, warmup_started_at, created_at)
                 VALUES ($1, $2, '203.0.113.250', $3, 0, NOW(), NOW(), NOW())",
            )
            .bind(format!("cov_status_impossible_{impossible}"))
            .bind(tenant)
            .bind(impossible)
            .execute(&h.pool)
            .await;
            assert!(
                rejected.is_err(),
                "'{impossible}' must violate dedicated_ips_status_check"
            );
        }

        // from_env is present only when the API token is configured.
        let previous = std::env::var("HETZNER_API_TOKEN").ok();
        std::env::set_var("HETZNER_API_TOKEN", "cov-token");
        std::env::remove_var("HETZNER_MTA_SERVER_ID");
        let configured = HetznerIpProvider::from_env(h.pool.clone());
        assert!(configured.is_some(), "a token configures the provider");
        std::env::remove_var("HETZNER_API_TOKEN");
        assert!(
            HetznerIpProvider::from_env(h.pool.clone()).is_none(),
            "no token, no provider"
        );
        match previous {
            Some(token) => std::env::set_var("HETZNER_API_TOKEN", token),
            None => std::env::remove_var("HETZNER_API_TOKEN"),
        }
    }
);
