...
async fn cleanup(pool: &PgPool) {
    let _ = sqlx::query("DELETE FROM events WHERE tenant_id LIKE 'test-tenant-%'").execute(pool).await;
    let _ = sqlx::query("DELETE FROM messages WHERE tenant_id LIKE 'test-tenant-%'").execute(pool).await;
    let _ = sqlx::query("DELETE FROM subscriptions WHERE tenant_id LIKE 'test-tenant-%'").execute(pool).await;
    let _ = sqlx::query("DELETE FROM tenants WHERE id LIKE 'test-tenant-%'").execute(pool).await;
    let _ = sqlx::query("DELETE FROM events WHERE tenant_id IN ('test-tenant-a', 'test-tenant-b')").execute(pool).await;
    let _ = sqlx::query("DELETE FROM tenants WHERE id IN ('test-tenant-a', 'test-tenant-b')").execute(pool).await;
}
...
async fn seed_tenants(pool: &PgPool) {
    ...
    for (id, plan, name) in &tenants {
        ...
        ).bind(id).bind(plan).bind(name).execute(pool).await;
    }
}

async fn seed_messages(pool: &PgPool) {
    let tenants = ["test-tenant-free","test-tenant-starter","test-tenant-pro","test-tenant-growth","test-tenant-enterprise"];
    ...
    for i in 0..total {
        ...
        let mut status = "sent";
        for (si, &w) in weights.iter().enumerate() { cum += w; if r < cum { status = statuses[si]; break; } }

        let _ = sqlx::query(
            "INSERT INTO messages (id, tenant_id, subject, status, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, $5) ON CONFLICT (id) DO NOTHING",
        ).bind(format!("msg-{i}")).bind(tenants[ti]).bind(format!("Msg {i}")).bind(status).bind(ts).execute(pool).await;
    }
}
...
async fn seed_events(pool: &PgPool) {
    ...
    for i in 0..total {
        ...
        let mut etype = "sent";
        for (ei, &w) in weights.iter().enumerate() { cum += w; if r < cum { etype = event_types[ei]; break; } }

        if has_tenant_id && has_recipient {
            let _ = sqlx::query(&format!(
                "INSERT INTO events (id, tenant_id, {type_col}, recipient, created_at)
                 VALUES ($1, $2, $3, $4, $5) ON CONFLICT (id) DO NOTHING"
            )).bind(format!("evt-{i}")).bind(tenants[ti]).bind(etype)
              .bind(format!("user{}@example.com", i % 1000)).bind(ts).execute(pool).await;
        } else if has_tenant_id {
            let _ = sqlx::query(&format!(
                "INSERT INTO events (id, tenant_id, {type_col}, created_at)
                 VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING"
            )).bind(format!("evt-{i}")).bind(tenants[ti]).bind(etype).bind(ts).execute(pool).await;
        } else {
            let _ = sqlx::query(&format!(
                "INSERT INTO events (id, {type_col}, created_at)
                 VALUES ($1, $2, $3) ON CONFLICT (id) DO NOTHING"
            )).bind(format!("evt-{i}")).bind(etype).bind(ts).execute(pool).await;
        }
    }
}
...
async fn seed_subscriptions(pool: &PgPool) {
    ...
    let subs = [
        ...
        ("test-tenant-enterprise","enterprise_annual","active",50000),
    ];
...
fn build_test_app(db: PgPool) -> axum::Router {
    ...
    let state = AppState::with_ddos_protector(
        ...
        Default::default(),
        None,
        Arc::new(ddos_protection::DdosProtector::new(ddos_protection::ProtectorConfig::default()).unwrap()),
        Arc::new(waf_engine::WafEngine::new(waf_engine::WafConfig::default())),
        None,
        None,
        api_server::resilience::ResilientClient::new_from_config(&api_server::config::Config::default()),
...
async fn tenant_dashboard_returns_correct_structure() {
    ...
    let resp = app.oneshot(Request::builder()
        .uri("/v1/analytics/dashboard?from=2020-01-01T00:00:00Z").method("GET")
...
async fn tenant_volume_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/analytics/volume?from=2020-01-01T00:00:00Z").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn tenant_engagement_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/analytics/engagement?from=2020-01-01T00:00:00Z").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn tenant_deliverability_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/analytics/deliverability?from=2020-01-01T00:00:00Z").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn admin_analytics_correct_structure() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics?range=7d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    ...
    if let Some(stats) = json.get("stats") {
        for f in &["totalSent","totalDelivered","totalOpened","totalClicked","totalBounced"] {
            assert!(stats.get(f).is_some(), "stats missing {}", f);
        }
    }
    assert!(json.get("timeSeries").is_some(), "admin analytics missing timeSeries");
    assert!(json.get("providers").is_some(), "admin analytics missing providers");
    cleanup(&db).await;
}
...
async fn admin_analytics_time_filtering() {
    ...
    for range in &["7d", "30d", "90d"] {
        let resp = app.clone().oneshot(Request::builder()
            .uri(&format!("/v1/admin/analytics?range={}", range)).method("GET")
            .header(header::AUTHORIZATION, "Bearer test-skip-auth")
            .body(Body::empty()).unwrap()).await.unwrap();

        assert_eq!(resp.status(), StatusCode::OK, "range {} must return 200", range);
        let body = axum::body::to_bytes(resp.into_body(), 1024*1024).await.unwrap();
        let json: Value = serde_json::from_slice(&body).unwrap();
        ...
        if *range == "7d" { counts_7d = sent; }
        if *range == "90d" { counts_90d = sent; }
    }

    assert!(counts_90d >= counts_7d, "90d counts ({counts_90d}) must be >= 7d ({counts_7d})");
    cleanup(&db).await;
}
...
async fn admin_analytics_requires_auth() {
    ...
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics?range=7d").method("GET")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_ne!(resp.status(), StatusCode::OK, "unauthorized must not return 200");
}
...
async fn delivery_analytics_correct_structure() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/delivery?range=7d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    ...
    for f in &["deliveryRate","bounceRate","complaintRate","totalSent","totalDelivered"] {
...
async fn delivery_latency_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/delivery/latency?range=7d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn delivery_provider_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/delivery/provider?range=7d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn delivery_queue_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/delivery/queue?range=7d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn growth_analytics_correct_structure() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/growth?period=30d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    ...
    assert!(json.get("activation").is_some(), "growth missing activation");
    assert!(json.get("engagement").is_some(), "growth missing engagement");
    cleanup(&db).await;
}
...
async fn growth_signup_timeline_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/growth/signups?period=30d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn growth_activation_funnel_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/growth/activation?period=30d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn growth_engagement_metrics_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/growth/engagement?period=30d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn predictive_analytics_correct_structure() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/predictive?window=90d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    ...
    let body = axum::body::to_bytes(resp.into_body(), 1024*1024).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    assert!(json.get("churnRisk").is_some(), "predictive missing churnRisk");
    assert!(json.get("capacity").is_some(), "predictive missing capacity");
    assert!(json.get("anomalies").is_some(), "predictive missing anomalies");
    cleanup(&db).await;
}
...
async fn predictive_churn_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/predictive/churn?window=90d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn predictive_capacity_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/predictive/capacity?window=90d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn predictive_anomalies_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/predictive/anomalies?window=90d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn insights_correct_structure() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/insights?lookback=7d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    ...
    assert!(json.get("recommendations").is_some(), "insights missing recommendations");
    cleanup(&db).await;
}
...
async fn insights_trends_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/insights/trends?lookback=7d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn insights_recommendations_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/analytics/insights/recommendations?lookback=7d").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn cross_tenant_health_correct_structure() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/cross-tenant/health").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    ...
    let body = axum::body::to_bytes(resp.into_body(), 1024*1024).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    assert!(json.get("platformHealthScore").is_some(), "missing platformHealthScore");
...
async fn cross_tenant_plans_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/cross-tenant/plans").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn cross_tenant_growth_endpoint_works() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/cross-tenant/growth").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    cleanup(&db).await;
...
async fn revenue_correct_structure() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/revenue").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
    ...
    assert!(json.get("monthlyData").is_some(), "revenue missing monthlyData");
    assert!(json.get("revenueByPlan").is_some(), "revenue missing revenueByPlan");
    cleanup(&db).await;
}
...
async fn admin_dashboard_correct_structure() {
    ...
    let app = build_test_app(db.clone());
    let resp = app.oneshot(Request::builder()
        .uri("/v1/admin/dashboard").method("GET")
        .header(header::AUTHORIZATION, "Bearer test-skip-auth")
        .body(Body::empty()).unwrap()).await.unwrap();
...
async fn delivery_rate_calculation_correct() {
    ...
    let now = Utc::now();
    for i in 0..100 {
        let status = if i < 85 { "delivered" } else if i < 93 { "bounced" } else { "opened" };
        let _ = sqlx::query(
            "INSERT INTO messages (id, tenant_id, subject, status, created_at, updated_at)
             VALUES ($1, 'test-tenant-pro', $2, $3, $4, $4) ON CONFLICT (id) DO NOTHING"
        ).bind(format!("calc-{}", i)).bind(format!("C{}", i)).bind(status).bind(now).execute(&db).await;
    }
    ...
    assert!((rate - 85.0).abs() < 1.0, "delivery rate should be ~85%, got {rate}%");
    cleanup(&db).await;
}
...
async fn bounce_rate_calculation_correct() {
    ...
    for i in 0..100 {
        ...
        let _ = sqlx::query(
            "INSERT INTO messages (id, tenant_id, subject, status, created_at, updated_at)
             VALUES ($1, 'test-tenant-pro', $2, $3, $4, $4) ON CONFLICT (id) DO NOTHING"
        ).bind(format!("bcalc-{}", i)).bind(format!("B{}", i)).bind(status).bind(now).execute(&db).await;
    }
    ...
    assert!((rate - 8.0).abs() < 1.0, "bounce rate should be ~8%, got {rate}%");
    cleanup(&db).await;
}
...
async fn mrr_calculation_correct() {
    ...
    if has_subs {
        ...
        assert!(total > 0, "MRR should be positive with seed data, got {total} cents");
    }
    cleanup(&db).await;
}
...
async fn tenant_isolation_no_data_leakage() {
    ...
    if has_events && has_tenant_id {
        ...
        for i in 0..200 {
            ...
            )).bind(format!("iso-{}", i)).bind(now - Duration::hours(i as i64)).execute(&db).await;
        }

        let count_a: i64 = sqlx::query_scalar("SELECT COUNT(*)::bigint FROM events WHERE tenant_id = 'test-tenant-a'")
            .fetch_one(&db).await.unwrap();
        let count_b: i64 = sqlx::query_scalar("SELECT COUNT(*)::bigint FROM events WHERE tenant_id = 'test-tenant-b'")
            .fetch_one(&db).await.unwrap();
...
async fn performance_queries_within_500ms() {
    ...
    for ep in &endpoints {
        ...
        let _body = axum::body::to_bytes(resp.into_body(), 1024*1024).await.unwrap();
        assert!(elapsed <= timeout, "endpoint {ep} took {elapsed:?}, expected <= {timeout:?}");
    }
