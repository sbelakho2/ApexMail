...
- Starter: $25, 50K emails, 5 domains, 5 team, 10K contacts, 30d retention
- Pro: $65, 150K emails, 25 domains, 10 team, 50K contacts, 60d retention
- Growth: $150, 500K emails, 100 domains, 25 team, 200K contacts, 90d retention
- Overage: $0.40/1,000 emails
"""
...
backup_path = 'data/train_agent.jsonl.bak2'

def fix_assistant_text(line_num, text):
    """Apply all fixes to assistant responses only. Returns (fixed_text, fixes_applied)."""
    fixes = []
    
    # Split into messages - we only modify assistant parts
    # Use a marker-based approach to selectively edit assistant sections
    ...
    for part in parts:
        ...
        # "Growth ($150/mo) supports 10" (after Pro) → "supports 25"
        ...
        # Only in Pro context (after Pro heading/mention)
        ...
        # "upgrading to Pro ($65/mo) for 50,000 emails"
        ...
        # In tables: "Includes 50,000 emails/month (you'd use 80%)" under Pro
        ...
        # Recalculate: 35,000 × $1.50/1K = $52.50 → N/A (no overages)
...
def rewrite_response_l533_l596(text):
    ...
    # Also fix: "Pro ($65/mo) which gives you 50,000 emails/month — nearly double"
    text = text.replace(
...
def rewrite_response_l783_l964(text):
    ...
    # The response says "you get 500,000 emails included" (correct) then calculates
    # "142,000 - 100,000 = 42,000 extra" (uses old 100K limit). Need to fix.
...
- Extra cost: **$0.00**"""
    ...
    # Fix the total line if it exists
    # Old total was $150 + $21 = $171, should just be $150
...
def rewrite_response_l226_l612(text):
    ...
    # This has completely wrong overage at $1.50/1K rate
    text = text.replace(
...
def rewrite_response_l115_l763(text):
    ...
    # Fix savings comparison if present
    text = text.replace(
        'PAYG saves you $17/month',
