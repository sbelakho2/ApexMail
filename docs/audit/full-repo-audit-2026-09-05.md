...
**Live-verified:** signup HTML contract (fields, grid, hint classes, aria, widget + challenge endpoint present, div balance 17/17, no always-on hint, no visible metacommentary — the only date string in the page source is an invisible JS comment); browser structure check (all controls in form, 2-col grid, focus hint none→block); widget solved state ("Verified." + "Verification complete." + token set).
