(() => {
  ...
  if (siteHeader) {
    const updateHeader = () => {
      if (window.scrollY > 20) {
        ...
        siteHeader.classList.add("bg-surface-50/95", "backdrop-blur-md", "border-surface-200", "shadow-premium-sm");
      } else {
        siteHeader.classList.remove("border-transparent", "shadow-premium-sm");
        siteHeader.classList.add("bg-surface-50/95", "backdrop-blur-md", "border-surface-200");
      }
    };
  ...
  function getStoredTheme() {
    return localStorage.getItem("apexmail-theme");
  }
  ...
  // Initialize toggle button visibility and label
  ...
  themeToggles.forEach((btn) => {
    const sun = btn.querySelector('[data-theme-icon="sun"]');
    const moon = btn.querySelector('[data-theme-icon="moon"]');
    if (sun) sun.classList.toggle("hidden", initialTheme === "dark");
    if (moon) moon.classList.toggle("hidden", initialTheme === "light");
