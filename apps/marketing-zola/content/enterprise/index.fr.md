+++
title = "ApexMail Enterprise | Conditions commerciales et achats"
description = "Email transactionnel Enterprise : un contrat annuel de €30,000 avec 5 millions d'emails inclus, un support dédié et une planification contractuelle du déploiement."
template = "prose.html"

[extra]
og_image = "/images/og-image.png"
+++

## ApexMail Enterprise

Enterprise est une offre contractuelle de livraison d'emails pour les organisations qui ont besoin d'une revue commerciale, de sécurité ou de déploiement au-delà des forfaits self-service publics.

## Droits du catalogue

| Élément | Enterprise |
|---|---|
| Prix | €30,000/an dans le cadre d'un contrat annuel |
| Volume d'emails inclus | 5,000,000 emails/mois |
| Appels API | Illimités |
| Domaines et membres d'équipe | Illimités |
| Rétention | 730 jours |
| IP dédiées | 10 incluses |
| Contrôles d'accès | SAML SSO et journaux d'audit |
| Niveau de support | Dédié |
| Cloud Privé / BYOIP | Soumis à revue contractuelle et technique |

Le prix annuel du catalogue est de €30,000, soit 10 fois le prix mensuel de référence de €3,000. Les coûts personnalisés de volume, de déploiement et de services professionnels sont négociés et indiqués uniquement dans une proposition écrite ou un bon de commande.

## Revue d'achat

Un acheteur qualifié peut demander la DPA actuelle, les informations sur les sous-traitants, la présentation de sécurité, la documentation d'architecture et les conditions de service applicables. La facturation, les bons de commande ou d'autres modalités de paiement ne sont disponibles que si elles sont approuvées au contrat.

## Sécurité et conformité

- ApexMail prend en charge la documentation de traitement orientée RGPD et propose une DPA à examiner.
- La disponibilité HIPAA et une BAA ne sont **pas proposées actuellement**.
- ApexMail n'est **pas actuellement certifié SOC 2**. Le travail interne de préparation ne constitue ni un droit produit, ni un engagement sur une date de certification.
- Tout SLA, périmètre de résidence des données, réponse à un questionnaire de sécurité ou engagement de déploiement privé est régi par l'accord signé.

## Cycle de facturation

Enterprise n'est pas achetable via le Checkout public générique. Le contrat ou l'arrangement de facturation approuvé fait foi pour les droits. Pour les forfaits self-service publics, Stripe Checkout et le webhook d'abonnement Stripe vérifié sont requis avant l'octroi d'un forfait payant.

## Contact

[Contactez l'équipe commerciale](/fr/contact/sales/) pour une revue commerciale, de sécurité et d'architecture factuelle.
