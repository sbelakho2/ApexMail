...
Ofrezca a sus clientes una infraestructura de email fiable y aislada sin construir ni mantener su propia capa de email. ApexMail da a cada uno de sus tenants sus propios dominios, claves de API, endpoints de webhooks con un secreto HMAC por tenant, listas de supresión, IP dedicadas y flujos de eventos.
...
1. Cree una cuenta para su plataforma.
2. Aprovisione un tenant independiente para cada cliente.
...
Cualquier disposición de despliegue no estándar requiere una revisión de arquitectura y contractual separada.
...
- Las claves de API tienen alcance limitado a su tenant. El acceso entre tenants se bloquea en la capa de autorización.
- Los endpoints de webhooks se configuran por tenant. Las firmas HMAC usan un secreto por tenant.
- Los registros de auditoría recogen toda creación, eliminación y cambio de permisos de tenants.

## Consideraciones de cumplimiento

- Cada tenant mantiene listas de supresión, autenticación de dominios y retención de eventos independientes.
- La cobertura de la DPA para los tenants requiere la DPA de la plataforma con ApexMail. La extensión contractual a los tenants es responsabilidad de la plataforma.
...
- El aislamiento de tenants es lógico en los planes de Nube compartida. Un despliegue contratado por separado puede definir requisitos adicionales de aislamiento, pero no constituye un derecho de un plan público.
- La analítica entre tenants requiere que la plataforma agregue externamente los datos de eventos de los tenants.
- La marca blanca está disponible en el plan Enterprise.

## Siguiente paso recomendado

[Contacte con ventas](/es/contact/sales/) para una revisión de arquitectura multi-tenant y precios por volumen para su despliegue.
