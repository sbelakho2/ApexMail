...
const SECRET_TYPES: Record<string, { label: string; icon: string; color: string }> = {
    api_key: { label: 'API Key', icon: '🔑', color: 'bg-blue-500/10 text-blue-700 border border-blue-500/20' },
    database: { label: 'Database', icon: '🗄️', color: 'bg-emerald-500/10 text-emerald-700 border border-emerald-500/20' },
    oauth: { label: 'OAuth', icon: '🔐', color: 'bg-violet-500/10 text-violet-700 border border-violet-500/20' },
    certificate: { label: 'Certificate', icon: '📜', color: 'bg-amber-500/10 text-amber-700 border border-amber-500/20' },
    encryption: { label: 'Encryption', icon: '🔒', color: 'bg-muted text-muted-foreground border border-border' },
};
        ...
        <div className="max-w-6xl mx-auto">
            ...
            {selectedSecret && (
                <div className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setSelectedSecret(null)}>
                    <div className="bg-card rounded-[18px] p-6 w-full max-w-lg shadow-xl border border-border" onClick={(e) => e.stopPropagation()}>
