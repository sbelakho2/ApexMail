...
export function ComparisonSection() {
  ...
  return (
    <section ref={ref} className="py-20 lg:py-32 relative">
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-surface-900/30 to-transparent" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {/* Table Header */}
          <div className="grid grid-cols-5 gap-4 p-4 lg:p-6 border-b border-surface-700 bg-surface-800/50">
            <div className="font-medium text-surface-400">Feature</div>
            <div className="text-center">
              <div className="font-bold text-primary-400">ApexMail</div>
              <div className="text-xs text-surface-500">Us</div>
            </div>
            <div className="text-center text-surface-300">
              <div className="font-medium">SendGrid</div>
              <div className="text-xs text-surface-500">Twilio</div>
            </div>
            <div className="text-center text-surface-300">
              <div className="font-medium">Mailchimp</div>
              <div className="text-xs text-surface-500">Intuit</div>
            </div>
            <div className="text-center text-surface-300">
              <div className="font-medium">AWS SES</div>
              <div className="text-xs text-surface-500">Amazon</div>
            </div>
          </div>
          ...
          {comparisonData.categories.map((category, categoryIndex) => (
            <div key={category.name}>
              {/* Category Header */}
              <div className="px-4 lg:px-6 py-3 bg-surface-800/30 border-b border-surface-700/50">
                <span className="text-sm font-semibold text-surface-400 uppercase tracking-wider">
                  {category.name}
                </span>
              </div>
              ...
              {category.features.map((feature, featureIndex) => (
                <motion.div
                  ...
                  className={cn(
                    'grid grid-cols-5 gap-4 px-4 lg:px-6 py-4 items-center',
                    featureIndex !== category.features.length - 1 && 'border-b border-surface-700/30'
                  )}
                >
                  <div className="text-sm text-surface-300">{feature.name}</div>
                  <div className="flex justify-center font-medium text-white">
                    {renderValue(feature.apexmail)}
                  </div>
                  <div className="flex justify-center text-surface-400">
