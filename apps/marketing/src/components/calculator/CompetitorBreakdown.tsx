...
const renderValue = (value: string | boolean) => {
  if (value === true) {
    return <CheckCircle className="w-5 h-5 text-emerald-600 mx-auto" strokeWidth={2} />;
  }
  if (value === false) {
    return <div className="w-1.5 h-1.5 rounded-full bg-surface-200 mx-auto" />;
  }
  if (typeof value === 'string' && (value.toLowerCase().includes('none') || value.toLowerCase().includes('manual'))) {
    return (
      <span className="text-surface-400 flex items-center justify-center gap-1.5 font-medium">
