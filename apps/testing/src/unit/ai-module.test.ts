...
describe('ThompsonSampler', () => {
    // Mock implementation to test the algorithm
    class ThompsonSamplerMock<T = string> {
...
describe('UCBBandit', () => {
    class UCBBanditMock<T = string> {
        ...
        select(): { armId: string; value: T; ucbValue: number } | null {
            ...
            // First, select any arm that hasn't been pulled
            for (const [id, arm] of this.arms) {
                if (arm.pulls === 0) {
            ...
            // UCB1 formula: mean + sqrt(2 * ln(t) / n)
