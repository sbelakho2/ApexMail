...
- **Nonce-bound IP binding** — the record stores `binding_tag`, an HMAC over the nonce + canonical IP bytes (IPv4-mapped IPv6 normalized to 4-byte IPv4). The tag changes with every nonce, so the record **creates no stable IP-derived identifier**. Verification recomputes the tag from the submitting IP and compares in constant time; an empty `binding_tag` (binding disabled) or a `None` client IP skips the check. This is a **relay mitigation, not a guarantee**: IPs legitimately change behind NAT/proxies, and operators can disable the check.
