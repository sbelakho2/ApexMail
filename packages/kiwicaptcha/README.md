...
- Widget UI: a modern, responsive widget with native dark mode and zero external dependencies (no third-party JS, no third-party iframes and no third-party hosts). An ordinary SHA-only solve renders no iframe; when the experimental ExecutionChallenge dimension is armed, the local interpreter runs in a short-lived same-origin sandboxed iframe. The emitted markup takes an optional CSP nonce: with a nonce, `<style nonce>` / `<script nonce>` are emitted; without one, the widget still works under CSP policies that allow `'unsafe-inline'` or where the application post-processes the HTML.
