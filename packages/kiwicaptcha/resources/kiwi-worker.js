...
(function () {
  ...
  var kiwiRuntimeSrc = null;
  var kiwiGluePromise = null;
  function ensureGlue() {
    if (!kiwiRuntimeSrc) return Promise.resolve(null);
    if (kiwiGluePromise) return kiwiGluePromise;
    kiwiGluePromise = new Promise(function (resolve) {
      ...
      initWasm().then(function (w) {
        ...
        if (typeof wasmProtocol !== "number" || wasmProtocol !== KIWI_SOLVER_PROTOCOL_VERSION) {
          post({ type: "failed", reason: w ? "protocol-mismatch" : "no_wasm" });
          resolve(null);
          return;
        }
  ...
  function solveMessage(m) {
    ...
    // Files mode: await the driver-supplied glue before any solve work —
    // the solve is queued behind the glue handshake, never racing it.
    Promise.resolve(kiwiRuntimeSrc ? ensureGlue() : null)
      .then(function () { return initWasm(); })
      .then(function (w) {
  ...
  // Startup handshake: BEFORE any solve work, verify the loaded wasm's
