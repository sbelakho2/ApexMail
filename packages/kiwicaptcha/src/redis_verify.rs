...
impl RedisChallengeStore {
    ...
    /// `Ok(Some(owner))` when the claim was taken, `Ok(None)` when the
    /// record is missing, not consumed-resultless, or already claimed by
    /// a live or unparseable owner. Mirrors the PHP
...
"#,
    ...
    /// a refused fence (`Ok(false)` on ownership-lost, no write) lets
