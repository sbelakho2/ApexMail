...
KiwiCaptcha from an incumbent provider (`reCAPTCHA` / `hCaptcha` /
`Turnstile`).

## Migrating from the legacy bundled Symfony layer

This bundle is the sole Symfony integration of KiwiCaptcha. Earlier
...
- `GET {prefix}/api.js?compat=recaptcha|hcaptcha|turnstile` is the
  ...
  controls (buttons / `data-size="invisible"`) execute on click, and
  `data-callback` / `data-expired-callback` / `data-error-callback` fire
  with the same token. The provider globals (`grecaptcha` / `hcaptcha` /
  `turnstile`) and the provider response fields
  (`g-recaptcha-response`, `h-captcha-response`, `cf-turnstile-response`)
  all share the one underlying Kiwi token.

This bridge is a migration bridge, not a universal drop-in replacement.
It covers the incumbent lifecycle surface the migrating pages actually
use, and the differences below are intentional.
...
- **Omitted-id semantics**: per Google's API, `reset()`,
  ...
  created widget; they are not a no-op and not the hidden-v3 path.
...
- **v3 (score) is intentionally not emulated.** Google v3's backend
  ...
  KiwiCaptcha's adaptive-risk policy. The sitekey maps to a scope with
  its own pre-issue and post-solve risk decisions, and the application
  validates the expected action server-side exactly as it validates any
  scope. The real sitekey and the requested action are transmitted
  independently in the challenge request. `execute(sitekey, {action})`
  renders a hidden v3 widget that sends both. The server resolves the
  (sitekey, action) pair to the security scope, rejecting unknown actions
  server-side, and the widget resolves with the real Kiwi token. The
...
`risk.sitekey_allowlist` maps a public sitekey to a scope, server-owned.
Unknown sitekeys stay scope names subject to `allowed_scopes` plus the
risk assessment, never a policy reduction.
