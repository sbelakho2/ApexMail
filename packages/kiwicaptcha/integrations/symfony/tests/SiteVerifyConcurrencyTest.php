...
final class SiteVerifyConcurrencyTest extends TestCase
{
    ...
    public function testOneHundredSimultaneousSiteverifyCallsSameTokenYieldsExactlyOneSuccess(): void
    {
        ...
        try {
            $probe = new \Predis\Client(self::redisTestUrl(), ['timeout' => 1.0, 'read_write_timeout' => 1.0]);
            $probe->ping();
        } catch (\Throwable) {
        ...
        for ($i = 0; $i < $workers; $i++) {
            ...
            if ($pid === 0) {
                ...
                $success = '0';
                try {
                    $client = new \Predis\Client(self::redisTestUrl(), ['timeout' => 2.0, 'read_write_timeout' => 2.0]);
    ...
    public function testOneHundredConcurrentSameIdempotencyKeyYieldsIdenticalResponses(): void
    {
        ...
        for ($i = 0; $i < $workers; $i++) {
            ...
            if ($pid === 0) {
                ...
                $line = 'error';
                try {
                    $client = new \Predis\Client(self::redisTestUrl(), ['timeout' => 15.0, 'read_write_timeout' => 15.0]);
                    $storage = new RedisStorage($client);
                    $controller = new SiteVerifyController(
                        new Verifier($storage),
                        self::SECRET,
    ...
    public function testSlowArgonSameIdempotencyKeyYieldsIdenticalResponses(): void
    {
        ...
        try {
            $probe = new \Predis\Client(self::redisTestUrl(), ['timeout' => 2.0, 'read_write_timeout' => 2.0]);
            $probe->ping();
        } catch (\Throwable) {
        ...
        for ($i = 0; $i < $workers; $i++) {
            ...
            if ($pid === 0) {
                ...
                $line = 'error';
                try {
                    $client = new \Predis\Client(self::redisTestUrl(), ['timeout' => 60.0, 'read_write_timeout' => 60.0]);
                    $storage = new RedisStorage($client);
                    $controller = new SiteVerifyController(
                        new Verifier($storage),
                        self::SECRET,
        ...
        $check = new \Predis\Client(self::redisTestUrl(), ['timeout' => 2.0, 'read_write_timeout' => 2.0]);
    ...
    public function testOwnershipLeaseCoversTheVerificationWindow(): void
    {
        ...
        if ($owner === 0) {
            ...
            $line = 'error';
            try {
                $client = new \Predis\Client(self::redisTestUrl(), ['timeout' => 20.0, 'read_write_timeout' => 20.0]);
    ...
    public function testRealRedisTwoConcurrentDifferentUuidsOnlyTheAtomicWinnerRecordsItsIdentity(): void
    {
        ...
        for ($i = 0; $i < $workers; $i++) {
            ...
            if ($pid === 0) {
                ...
                $line = 'error';
                try {
                    $client = new \Predis\Client(self::redisTestUrl(), ['timeout' => 5.0, 'read_write_timeout' => 5.0]);
                    $storage = new RedisStorage($client);
