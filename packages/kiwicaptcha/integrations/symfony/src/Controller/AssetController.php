 ...
 * `GET {prefix}/assets/{name}.{sha256-12}.{js|css}` returns the exact bytes
 ...
 * one cached copy across pages. The hash in the URL is the sha256 of
 * the content truncated to 12 hex characters, and an unknown hash is
 * a 404. A page can only reference the exact bytes the server
 * serves, so a mismatch can never pair a stale hash with different
 * content.
 *
