                    ...
                    ->children()
                        ...
                        ->arrayNode('calibration')
                            ...
                            ->children()
                                ...
                                ->end()
                                ->enumNode('mode')
                                    ->info('Label-selection contract: "complete" = every eligible outcome is labeled and fed to the calibrator; "random_sample" = Kiwi samples at assessment time (the receipt carries a "sampled" flag) and unsampled confirmations are DISCARDED, so the label can never select itself into the calibration population (a decision confirmed precisely because it was scored badly would bias the aggregates — sampling keeps the labels representative); "weighted" = the application supplies the inverse sampling probability per confirmation (RiskGateway::confirmDecisionOutcome($ppm) converts it to weight = 1_000_000/ppm) so labels with known selection bias are re-weighted into the population.')
