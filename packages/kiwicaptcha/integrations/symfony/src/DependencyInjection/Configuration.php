                ...
                ->arrayNode('risk')
                    ...
                    ->children()
                        ...
                        ->arrayNode('calibration')
                            ->info('Privacy-preserving aggregate calibration: score-bucket statistics (no identity, no IP, no pseudonym) with a bounded scope-bias adjustment. min_samples / max_adjustment / max_change_per_minute are passed to the Redis-backed AggregateCalibrator to bound the automatic adjustment; retention_secs is accepted for backward compatibility with the former in-memory calibrator and is informational (the Redis-backed store uses the risk-v1 contract\'s fixed 24 h window / 300 s receipts).')
                            ->addDefaultsIfNotSet()
                            ->children()
