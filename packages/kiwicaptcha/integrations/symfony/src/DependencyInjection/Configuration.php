        ...
        $root
            ->children()
                ...
                ->booleanNode('allow_local_global_limit_fallback')
                    ->info('Explicitly accepts the non-Redis issuance rate limiter in production when rate_limit_global > 0 but no Redis client is wired. The deployment-wide claim then degrades to a weaker window: a shared PSR-6 pool (rate_limit_cache) is cross-request best-effort, while the object-memory fallback is long-lived-runtime-only (a persistent worker such as RoadRunner/Swoole/amphp, or a single CLI process) and gives no cross-request protection under conventional PHP-FPM, where each request rebuilds the limiter. Production temporal limiting requires Redis or a genuinely persistent or shared PSR-6 pool, and the aggregate of N workers can approach N times the configured global cap. Default false: production refuses the combination.')
                    ->defaultFalse()
                ->end()
