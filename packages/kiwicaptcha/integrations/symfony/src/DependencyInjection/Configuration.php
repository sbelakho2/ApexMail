        ...
        $root
            ->children()
                ...
                ->booleanNode('allow_local_global_limit_fallback')
                    ->info('Explicitly accepts the process-local or best-effort issuance rate limiter in production when rate_limit_global > 0 but no Redis client is wired. The deployment-wide claim then degrades to the local window: the aggregate of N workers can approach N times the configured global cap. Default false: production refuses the combination.')
                    ->defaultFalse()
                ->end()
