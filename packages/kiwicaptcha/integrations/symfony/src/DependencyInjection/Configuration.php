            ...
            ->children()
                ...
                ->arrayNode('risk')
                    ...
                    ->children()
                        ...
                        ->end()
                        ->arrayNode('chaining')
                            ->info('SELECTIVE CHAINED CHALLENGES: an initial inexpensive proof, followed by a risk reassessment at form-submission time, and — when the reassessment demands a stronger stage than the first-stage profile (e.g. an Argon action or StepUp) — a signed one-shot CHAIN TICKET that gates a second (stronger) challenge issuance. The chain identifier, the previous challenge result and the policy epoch are signed/server-held, so a client cannot skip stages or re-run the same stage. The chain ticket is consumed atomically at the second issuance; a ticket-bearing request is NEVER downgraded to an unchained issuance.')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->booleanNode('enabled')
                                    ->info('Enable selective chaining. When true, a valid verification whose post-solve reassessment demands a stronger stage returns the kiwi.chain_required violation carrying the signed chain ticket (violation parameter {{ chain_ticket }}); the application re-renders the widget with data-kiwi-chain-ticket=<ticket> and the next challenge request presents it for a one-shot stage-2 issuance. Requires risk.enabled (the reassessment rides the risk gateway).')
                                    ->defaultFalse()
                                ->end()
                                ->integerNode('ttl_secs')
