                ...
                ->arrayNode('risk')
                    ...
                    ->children()
                        ...
                        ->scalarNode('result_receipt_signing_key')
                            ->info('OPTIONAL base64 32-byte Ed25519 seed (audit #80): when configured, the validator signs every valid verification result into an asymmetric receipt ({jti, scope, binding, outcome, issued_at_ms}) with sodium_crypto_sign_detached, exposed via KiwiCaptchaValidator::verifiedReceiptPayload() / ::verifiedReceiptSignature(). The result verification itself stays CENTRAL-ONLY (the HMAC secret never leaves the server); this key only enables EXPORTED result receipts that third parties verify with the PUBLIC key derived from this seed (never the private key).')
                            ->defaultNull()
                        ->end()
                        ->integerNode('max_challenges_per_scope_per_minute')
                            ->info('PER-SCOPE issuance cap (audit #89, default 0 = unlimited): when > 0, a Redis fixed-window counter `{kiwi:<ns>}:issuance:<scope>:<minute>` (INCR + EXPIRE 60 in one atomic Lua script) bounds how many challenges a scope may issue per minute — a public site key + claimed origin can no longer create unlimited billed verification work per scope. The cap is only enforceable with a Redis client (fail-fast at compile time otherwise); each scope gets its own independent window.')
                            ->defaultValue(0)
                            ->min(0)
                        ->end()
