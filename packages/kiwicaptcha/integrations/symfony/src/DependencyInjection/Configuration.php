                ...
                ->arrayNode('risk')
                    ...
                    ->children()
                        ...
                        ->integerNode('max_challenges_per_scope_per_minute')
                            ->info('PER-SCOPE issuance cap (audit #89, default 0 = unlimited): when > 0, a Redis fixed-window counter `{kiwi:<ns>}:issuance:<hex(hmac_sha256(scope, HKDF("kiwi/v2/scope-rate", master_secret)))>:<minute>` (INCR + EXPIRE 60 in one atomic Lua script) bounds how many challenges a scope may issue per minute — a public site key + claimed origin can no longer create unlimited billed verification work per scope. The RAW SCOPE STRING IS NEVER A REDIS KEY COMPONENT (audit #112): the scope is attacker-controlled with unbounded cardinality, so the key carries the keyed pseudonym derived from risk.master_secret (falling back to secret_key). The cap is only enforceable with a Redis client (fail-fast at compile time otherwise); each scope gets its own independent window.')
                            ->defaultValue(0)
                            ->min(0)
                        ->end()
