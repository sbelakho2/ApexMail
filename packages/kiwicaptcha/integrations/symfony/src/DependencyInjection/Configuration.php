            ...
            ->children()
                ...
                ->arrayNode('risk')
                    ...
                    ->children()
                        ...
                        ->end()
                        ->arrayNode('scopes')
                            ->info('Per-scope policy. The KEY is the application scope string (the one passed to the challenge endpoint / form "scope" option); "id" is the int scope identifier stored in Redis state — it MUST stay stable once deployed (defaults to crc32 of the scope name), two scopes must never share an id. Scopes NOT listed here are handled by the unknown_scope node (reject = the adaptive engine declines to evaluate and the controller issues the default profile; minimum = synthetic policy base_risk 100 / minimum sha20 / degraded sha20).')
                            ...
                            ->arrayPrototype()
                                ->children()
                                    ...
                                    ->booleanNode('post_solve_check')
                                        ->info('When true, a VALID solve triggers a fresh POST-SOLVE assessment (SolveSuccess) with the same context; a Deny there fails the validation with the "kiwi.post_solve_rejected" error, and the outcome is recorded as ConfirmedLegitimate / ConfirmedAbuse feedback with the post-solve decision id.')
                                        ->defaultValue(false)
                                    ->end()
                                ->end()
                            ->end()
                        ...
                        ->arrayNode('unknown_scope')
                            ->info('Behavior for scopes that are not configured. reject (default): the adaptive engine declines to evaluate and the controller issues the default profile (baseline Kiwi verification still applies). minimum: unknown scopes are treated with a synthetic policy (base_risk 100, minimum Sha20, degraded Sha20, ladder defaults).')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->enumNode('mode')->values(['reject', 'minimum'])->defaultValue('reject')->end()
                            ->end()
                        ->end()
                        ...
                        ->arrayNode('calibration')
                            ->info('Privacy-preserving aggregate calibration: score-bucket statistics (no identity, no IP, no pseudonym) with a bounded scope-bias adjustment. The Redis-backed store uses the risk-v1 contract\'s fixed bounds (24 h window, bias clamp -200..+200, 300 s receipts); the retention_secs / min_samples / max_adjustment / max_change_per_minute knobs below are accepted for backward compatibility with the former in-memory calibrator and are currently informational.')
                            ->addDefaultsIfNotSet()
                            ->children()
