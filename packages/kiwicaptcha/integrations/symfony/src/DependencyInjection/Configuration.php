        ...
        $root
            ->children()
                ...
                ->integerNode('rate_limit_window_secs')
                    ->info('Sliding-window size (seconds) for the issuance rate limits (per-client and global). The global limiter is bucketed per window-second, so its Redis cardinality is bounded by this value — the default 60 means at most 60 buckets ever exist, regardless of request volume.')
                    ->defaultValue(60)
                    ->min(1)
                ->end()
