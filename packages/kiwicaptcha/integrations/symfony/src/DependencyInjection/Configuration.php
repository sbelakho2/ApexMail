                ...
                ->arrayNode('risk')
                    ...
                    ->children()
                        ...
                        ->integerNode('subnet_epoch_secs')
                            ->info('Epoch length in seconds for the SUBNET pseudonym (default 900; subnet = IPv4 /24, IPv6 /56).')
                            ->defaultValue(900)
                            ->min(60)
                        ->end()
                        ...
                        ->integerNode('policy_version')
                            ->info('Version tag written into every decision (log/metrics only — the risk-v1 contract keeps it informational). Bump it when you retune the policy so logs remain comparable.')
                            ->defaultValue(1)
                            ->min(1)
                        ->end()
                        ->arrayNode('global_floors')
                            ->info('Minimum action per global pressure level 1..4 (default: 1=>sha16, 2=>sha18, 3=>sha20, 4=>sha20). The deployment-wide state script raises/lowers the level; the floor guarantees the aggregate posture tightens before per-source signals do.')
                        ...
                        ->end()
                        ->arrayNode('scopes')
                            ->info('Per-scope policy. The KEY is the application scope string (the one passed to the challenge endpoint / form "scope" option); "id" is the int scope identifier stored in Redis state — it MUST stay stable once deployed (defaults to crc32 of the scope name), two scopes must never share an id, and an unknown scope falls back to base_risk 100 / minimum allow / degraded allow while still being observed.')
                            ...
                            ->arrayPrototype()
                                ->children()
                                    ...
                                    ->booleanNode('post_solve_check')
                                        ->info('Reserved by the risk-v1 contract for post-solve re-assessment (the bundle records solve outcomes but does not re-challenge); kept in the policy so cross-language deployments share one contract.')
                                        ->defaultValue(false)
                                    ->end()
                                ->end()
                            ->end()
                            ->defaultValue([
                                'login' => ['id' => null, 'base_risk' => 100, 'minimum' => 'allow', 'degraded' => 'allow', 'post_solve_check' => false],
                                'signup' => ['id' => null, 'base_risk' => 100, 'minimum' => 'allow', 'degraded' => 'allow', 'post_solve_check' => false],
                            ])
                        ->end()
                        ->arrayNode('continuity_cookie')
                            ->info('First-party session continuity cookie: a random 16-byte nonce (hex, HttpOnly, SameSite=Lax) that links requests from the same browser into the risk engine\'s "session" signal. It carries NO identity — it is a fresh random value, and the engine only ever stores its keyed pseudonym.')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->scalarNode('name')
                                    ->defaultValue('kiwi_risk_session')
                                    ->cannotBeEmpty()
                                ->end()
                                ->integerNode('ttl_secs')
                                    ->info('Cookie lifetime (default 180 days; 0 = session cookie).')
