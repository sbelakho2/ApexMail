    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $root
            ->children()
                ...
                ->end()
                ->scalarNode('issuer')
                    ->info('Round 28: deployment issuer stamped into every issued challenge (e.g. "auth-prod"). When set, the verifier REJECTS any record whose issuer does not match exactly — a dev/staging/prod mixup cannot validate cross-environment. The core long supported issuer; this makes it first-class bundle configuration.')
                    ->defaultNull()
                ->end()
                ->integerNode('kid')
                    ->info('Round 28: signing key id (1..4294967295) stamped into every issued challenge. Paired with secrets_by_kid + revoked_kids this is the controlled HMAC-key rotation control: bump kid + add the new secret to secrets_by_kid; the old secret remains valid for verification until its kid is revoked.')
                    ->min(1)
                    ->max(4294967295)
                    ->defaultValue(1)
                ->end()
                ->arrayNode('secrets_by_kid')
                    ->info('Round 28: verification-only secrets for historical signing key ids (map of kid => secret, each >= 16 bytes). With the CURRENT signing secret in secret_key and kid matching, key rotation is: add the old secret here, bump kid + update secret_key with the new one. Verification of records signed under old kids uses these secrets.')
                    ->scalarPrototype()->end()
                ...
                ->arrayNode('revoked_kids')
                    ->info('Round 28: signing key ids whose verification is REVOKED (emergency key-compromise response). A record signed under a revoked kid is rejected with UnknownKid IMMEDIATELY — even when its secret is present in secrets_by_kid. Revocation always wins over rotation.')
                ...
                ->arrayNode('risk')
                    ...
                    ->children()
                        ...
                        ->integerNode('security_epoch_cache_secs')
                            ->info('Cache TTL of the SecurityEpochMonitor\'s central security-policy read (audit #81, default 1 s): the monitor reads `{kiwi:<ns>}:security-policy` hash\'s `min_policy_epoch` at most once per window, keeps a MONOTONIC in-process max (a regressed central value is ignored) and serves the last-observed epoch when Redis is unavailable. The bounded revocation latency of a policy bump is one cache window.')
                            ->defaultValue(1)
                            ->min(1)
                            ->max(30)
                        ->end()
                        ->integerNode('security_epoch_max_stale_secs')
                            ->info('MAX-STALE FAIL-CLOSED window of the SecurityEpochMonitor (audit #108, default 60 s, min 10 s): after the last SUCCESSFUL central policy read, once now > last_success + max_stale the monitor reports stale — the cached epoch may be outdated (an emergency revocation could have landed while the node could not read). While stale, the validator fails verification closed (temporary_unavailable — the token is not burned, the server refuses to trust its own cache) and the challenge controller refuses issuance with 503 SERVICE_UNAVAILABLE. Within the window the cached max keeps serving (bounded outage tolerance). The availability trade-off is deliberate: a node that cannot confirm the central policy for max_stale seconds stops issuing and stops verifying, rather than serving potentially-revoked challenges forever.')
                            ->defaultValue(60)
                            ->min(10)
                        ->end()
                        ->scalarNode('result_receipt_signing_key')
                            ->info('OPTIONAL base64 32-byte Ed25519 seed (audit #80): when configured, the validator signs every valid verification result into an asymmetric receipt ({jti, tenant, action, request_binding, issued_at, expires_at, issuer} — the full replay-critical set, audit #106) with sodium_crypto_sign_detached, exposed via KiwiCaptchaValidator::verifiedReceiptPayload() / ::verifiedReceiptSignature(). The result verification itself stays CENTRAL-ONLY (the HMAC secret never leaves the server); this key only enables EXPORTED result receipts that third parties verify with the PUBLIC key derived from this seed (never the private key). Signature verification alone is NOT sufficient for single-use actions: the integrator must atomically record the jti (INSERT IF NOT EXISTS / SET NX) and treat a pre-existing jti as a replay (verify_and_consume — README).')
                            ->defaultNull()
                        ->end()
                        ->integerNode('max_challenges_per_scope_per_minute')
                            ->info('PER-SCOPE issuance cap (audit #89, default 0 = unlimited): when > 0, a Redis fixed-window counter (INCR + EXPIRE 60 in one atomic Lua script) bounds how many challenges a scope may issue per minute — a public site key + claimed origin can no longer create unlimited billed verification work per scope. The quota keys on the SERVER-OWNED scope identity (audit round 15): the configured risk.scopes.<name>.id (stable u32), or the shared synthetic unknown-scope id — the raw scope string is NEVER a Redis key component (audit #112), and the quota namespace is bounded by the server-owned set when risk.allowed_scopes is configured. The cap is only enforceable with a Redis client (fail-fast at compile time otherwise); each scope gets its own independent window.')
                            ->defaultValue(0)
                            ->min(0)
                        ->end()
                        ->arrayNode('allowed_scopes')
                            ->info('SERVER-OWNED SCOPE ALLOWLIST (audit round 15): the list of application scope strings the challenge endpoint may issue for. Empty (default) = any syntactically valid scope is accepted (backward compatible). When NON-EMPTY, a scope outside the list is refused with 422 SCOPE_NOT_ALLOWED BEFORE the risk assessment and the quota checks — the per-scope issuance cap then operates over the server-owned namespace (bounded cardinality), which is the only configuration in which that cap is an independent security bound. Unknown scopes listed here but absent from the risk.scopes map are still assessed under the unknown_scope policy (minimum by default); adding the scope to risk.scopes gives it its own stable canonical id.')
                            ->scalarPrototype()
                                ->validate()
                        ...
                        ->end()
                        ->arrayNode('sitekeys')
                            ->info('ROUND 30 (item 14): server-owned v3-style sitekey policy — public sitekey -> {default_scope, actions: {action: scope}}. The BROWSER sends sitekey + action in the challenge request; the server resolves (sitekey, action) -> security scope, so a client can never invent protected scope names. An unknown action under a configured sitekey is REJECTED (never silently mapped). Sitekeys without a config entry resolve through sitekey_allowlist / the plain scope. The per-sitekey "binding" option was REMOVED in round 31 — the core binds issuance by the GLOBAL binding_mode only, so a per-sitekey "required"/"none" claim could not be enforced (a misleading security promise); the global server-owned mode is the only binding control.')
                            ->useAttributeAsKey('sitekey')
                            ->arrayPrototype()
                                ->children()
