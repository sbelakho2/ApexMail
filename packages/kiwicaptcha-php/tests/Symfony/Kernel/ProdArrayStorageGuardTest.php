...
use KiwiCaptcha\Symfony\DependencyInjection\KiwiCaptchaExtension;
...
final class ProdArrayStorageGuardTest extends TestCase
{
    private function load(string $environment): ContainerBuilder
    {
        ...
        (new KiwiCaptchaExtension())->load([['secret' => str_repeat('a', 32)]], $container);
    ...
    {
        self::assertTrue($this->load('test')->hasDefinition('kiwicaptcha.storage.array'));
        self::assertTrue($this->load('dev')->hasDefinition('kiwicaptcha.storage.array'));
    }
    ...
    public function testCustomStorageServiceIdAllowedInProd(): void
    {
        ...
        (new KiwiCaptchaExtension())->load(
            [['secret' => str_repeat('a', 32), 'storage' => 'my.shared.storage']],
            $container,
        );

        self::assertFalse($container->hasDefinition('kiwicaptcha.storage.array'));
    }
